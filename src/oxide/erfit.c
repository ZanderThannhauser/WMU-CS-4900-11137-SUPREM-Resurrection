...
void erfc_growth(float temp, int ornt, int oxhow, double dt) {
    ...
    for (in = 0; in < nn; in++) {
        ...
        if (nd[in]->mater == Si && node_mat(in, SiO2) < 0)
            continue;
 ...
 *----------------------------------------------------------------------*/
void fixup_t1d(float cutback) { t1d = old_t1d + (t1d - old_t1d) * cutback; }

/*-----------------ERF1_GROWTH------------------------------------------
 * Another horrible little analytic approximation, from
 * IEEE TED-34 May 1987
 *----------------------------------------------------------------------*/
void erf1_growth(float temp, int ornt, int oxhow, double dt, int which) {
    int in, /* node walker */
        syvel = imptosol[YVEL];
        ...
        /* O_ERF2 */
            ...
            if (yn > -1e-9) {
                if (xn > erf1_delta) {
                    ...
                    float d1p =
                        erf1_delta - erf1_q * (lift_ratio / (lift_ratio - 1));
