...
#define fetchd(N, V, S, A, p1, p1n, p2, p2n)                                   \
    if (is_specified(param, N)) {                                              \
        if (p1 >= 0 && p2 >= 0)                                                \
            ...
            fprintf(stderr,                                                    \
                    "When specifying %s you must also specify %s%s%s\n", N,    \
                    p1n, (*p2n) ? " and " : "", p2n);                          \
    }
#define chosen(A) is_specified(param, A) && get_bool(param, A)
    ...
    if (given("alpha")) {
        if (mat >= 0 && mat2 >= 0) {
            ...
            fprintf(stderr, "When specifying alpha you must also specify "
                            "materials 1 and 2\n");
    }
    ...
    if (given("orient"))
        ...
        case 111:
            ...
            break;
        default:
            fprintf(stderr, "Unknown orientation %d\n", ornt);
            ornt = -1;
            break;
        }
    ...
    if (given("ori.dep"))
        ornt_dep = get_bool(param, "ori.dep");

    /* Stress dependence */
    if (given("stress.dep"))
        stress_dep = get_bool(param, "stress.dep");
    fetch("Vc", Vc, 1e-24, 0);
    fetch("Vr", Vr, 1e-24, 0);
    fetch("Vd", Vd, 1e-24, 0);
    fetch("Vt", Vt, 1e-24, 0);
    ...
    spes = -1;
    if (get_bool(param, "dry")) {
        dw = 0;
        spes = O2;
    } /* maybe :-) */
    if (get_bool(param, "wet")) {
        dw = 1;
        spes = H2O;
    }
    ...
    fetchd("thinox.0", thox0[dw][ornt], 1e-4 / 60, 0, spes, "wet/dry", ornt,
           "orientation");
    fetchd("thinox.e", thoxe[dw][ornt], 1.0, 0, spes, "wet/dry", ornt,
           "orientation");
    fetchd("thinox.l", thoxl[dw][ornt], 1e-4, 0, spes, "wet/dry", ornt,
           "orientation");

    /* The Henry's law constant */
    fetchd("henry.coeff", HN(spes, mat), 1.0, 0, spes, "wet/dry", mat,
           "material");
    ...
    if (given("seg.0")) {
        if (spes >= 0 && mat >= 0 && mat2 >= 0) {
            ...
            fprintf(stderr,
                    "When specifying seg.0 you must also specify wet/dry and "
                    "materials 1 and 2\n");
    }
    if (given("seg.E")) {
        if (spes >= 0 && mat >= 0 && mat2 >= 0) {
            ...
            fprintf(stderr,
                    "When specifying seg.E you must also specify wet/dry and "
                    "materials 1 and 2\n");
    }
    fetchd("trn.0", Trn0(spes, mat2, mat) = Trn0(spes, mat, mat2), 1, 0, spes,
    ...
    *nfla = 0;
    ...
    for (walk = strtok(input, sep); walk; walk = strtok(NULL, sep)) {
        ...
        if (*nfla >= mfla) {
            if (!fla) {
                mfla = 128;
                fla = salloc(float, mfla);
            } else {
                mfla *= 2;
                fla = sralloc(float, mfla, fla);
            }
        }
...
int mono_array(float *fla, int nfla, char *name) {
    ...
    for (i = 0; i < nfla - 1; i++)
        if (fla[i] >= fla[i + 1]) {
            if (name)
                fprintf(stderr, "Array %s is not in ascending order\n", name);
            return (0);
        }
    return (1);
}
