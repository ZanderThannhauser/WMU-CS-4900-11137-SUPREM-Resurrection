    ...
    for (ie = 0; ie < FEne; ie++) {
        ...
        for (j = 0; j < nface; j++) {
            ...
            if (dirs[X] + dirs[Y] != 1 && nbj < BC_OFFSET + 1) {
                fprintf(stderr,
                        "Warning: this neumann face is not parallel to one "
                        "coordinate axis!");
 ...
 *----------------------------------------------------------------------*/
void FEdirichlet(char *par, struct par_str *param) {
    int ie, j, i, k, v, c;
    FEelt_typ *ae;
    FEnd_typ *an;
            ...
            for (j = 0; j < 3; j++) {
                ...
                if (ae->face[j] == BC_OFFSET + c) {
                    ...
                    for (k = 1; k <= 3; k++) {
                        ...
                        fmacro("x", an->cord[X], "%e");
                        fmacro("y", an->cord[Y], "%e");
...
void face_ache(FEelt_typ *ae, int j) {
    ...
    ip = ae->nd[j + 3];
    FEnd[ip]->garbage[X] =
        0.5 * (FEnd[ip1]->garbage[X] + FEnd[ip2]->garbage[X]);
    FEnd[ip]->garbage[Y] =
        0.5 * (FEnd[ip1]->garbage[Y] + FEnd[ip2]->garbage[Y]);
        ...
        for (j = 0; j < 3; j++) {
            if (ae->face[j] >= 0 && FEelt[ae->face[j]]->mat == SiO2) {
                ...
                for (k = tface[j], kk = k + 3; k < kk; k++) {
                    ...
                    an->garbage[2 + X] = -(an->sig[XX] * an->garbage[X] +
                                           an->sig[XY] * an->garbage[Y]);
                    an->garbage[2 + Y] = -(an->sig[XY] * an->garbage[X] +
                                           an->sig[YY] * an->garbage[Y]);
                }
            }
        }
    }
...
void clear_bc() {
    ...
    for (in = 0; in < FEnn; in++) {
        an = FEnd[in];
        for (j = an->sol, jj = j + FEdf; j < jj;)
            *j++ = 0;
        for (j = an->bc, jj = j + FEdf; j < jj;)
            *j++ = 0;
        for (k = an->fixity, kk = k + FEdf; k < kk;)
            *k++ = 0;
    }
}
