...
int mk_reg(int mat) {
    if (nreg + 1 > MAXREG)
        panic("Too many edges generated");
    reg[nreg] = (reg_typ *)malloc(sizeof(reg_typ));
    if (!reg[nreg])
        panic("Out of storage in alloc_region");

    reg[nreg]->mater = mat;

    reg[nreg]->edg.num = reg[nreg]->edg.all = 0;
    reg[nreg]->edg.list = NULL;
    reg[nreg]->tri.num = reg[nreg]->tri.all = 0;
    reg[nreg]->tri.list = NULL;
    reg[nreg]->fac.num = reg[nreg]->fac.all = 0;
    reg[nreg]->fac.list = NULL;
    reg[nreg]->nd.num = reg[nreg]->nd.all = 0;
    reg[nreg]->nd.list = NULL;
    reg[nreg]->sreg = -1;
    reg[nreg]->flags = 0;
