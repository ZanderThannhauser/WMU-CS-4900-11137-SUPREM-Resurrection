...
void beta_calc(int nv, double *aqinvr, double *ap, double *norm);

void update_body(int nv, double alpha, double *ans, double *p, double *ap,
                 double *qinvr, double *r, double *nm);

void alpha_body(int nv, double *ap, double *r, double *normap, double *rap);

void elim_blk_row(int nv, int nsol, int *sol, int sj, int si, double *tmp);

void cg_block(int nv, int nsol, int *sol, int *il, int loff, double *l,
              double **ans, int initial);
...
void cg_block(int nv, int nsol, int *sol, int *il, int loff, double *l,
    ...
    while (count < methdata.blkitlim) {
        ...
        if ((maxnorm < (nm2 * 1.0e-8)) || (maxnorm < 1.0e-4))
            return;
        ...
        for (i = 0; i < nsol; i++) {
            si = sol[i];
            upd_p(nv, p[str][si], p[curr][si], qinvr[si], ap[str][si],
                  ap[curr][si], aqinvr[si], beta[curr]);
        }

        for (b = 0; b < curr; b++) {
            for (i = 0; i < nsol; i++) {
                si = sol[i];
                hist_upd(nv, p[str][si], p[b][si], ap[str][si], ap[b][si],
                         beta[b]);
            }
        }
        count++;
