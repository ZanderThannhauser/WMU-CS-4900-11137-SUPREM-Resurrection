	...
	float min_value)
{
    ...
    for (i = 0; i < ne; i++) {
         ...
         *  vertices
         */
        if (((nd[(tri[i])->nd[0]])->mater != mat_num) && (mat_num != 7))
            continue;
        min_xindex = xsize;
        min_yindex = ysize;
        max_xindex = 0;
        ...
        for (j = 0; j < 3; j++) {
            ...
            window(pt[point_num]->cord[0], pt[point_num]->cord[1], xdata, ydata,
                   xsize, ysize, &x_lo, &x_hi, &y_lo, &y_hi);
        ...
        for (j = min_xindex; j <= max_xindex; j++) {
            if ((j < win_xmin_index) || (j > win_xmax_index))
                continue;
            for (k = min_yindex; k <= max_yindex; k++) {
                if ((k < win_ymin_index) || (k > win_ymax_index))
                    continue;
                index = (k - win_ymin_index) *
                            (win_xmax_index - win_xmin_index + 1) +
                        (j - win_xmin_index);
                ...
                if (pt_in_tri(c, i)) {
                    ...
                    /* for multiple nodes, take value with maximum absolute
                       value, unless
                            one of the nodes is silicon.  then use the silicon
                       value */
                    if (pt_num != -1) {
                        if (si_interface(pt_num)) {
                            ...
                            if (fabs((double)value) > fabs((double)data[index]))
                                data[index] = value;
                        }
                    } else
                        data[index] = value;
...

{
    ...
    answer = cords[0][2] +
             ((cords[0][0] - x) * normal_x + (cords[0][1] - y) * normal_y) /
                 normal_z;
