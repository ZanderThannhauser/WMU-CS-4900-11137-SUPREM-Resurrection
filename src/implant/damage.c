...
void setdam(int imp, double energy, double* damstat)
{
    ...
    if (((imp == B) && (energy > 20.0)) || ((imp == P) && (energy > 55.0))) {
        c1 = a1 * (exp(z0 / a1) - 1) +
             a3 * sqpi * exp(z1 / a1) * ERFC((z0 - a2) / (sq2 * a3));
        c1 = 1.0 / c1;
        c2 = c1 * exp(z1 / a1);
        wt = 0.0;
    } else if ((imp == B) || (imp == P)) {
        ...
        c2 = -a1 * exp((z0 - z1) / a1) +
             a3 * sqpi *
                 (2.0 - ERFC(a2 / (sq2 * a3)) - ERFC((z0 - a2) / (sq2 * a3)));
        c2 = 1.0 / c2;
        c1 = c2 * exp(-z1 / a1);
        wt = 1.0;
    }
 ...
 *									*
 ************************************************************************/
double dam_vert(depth, damstat) double depth;
double *damstat;
{
    double val;
 ...
 *									*
 ************************************************************************/
double dam_lat(pos, left, right, data) double pos, left, right;
