...
#include "./src/suprem/include/expr.h" /*for last_temp*/
 ...
 *----------------------------------------------------------------------*/
int ig2_read(char *name, int flip, float scale) {
	...
	scal[0] = 1e-4 * scale, scal[1] = 1e-4 * ((flip) ? -scale : scale),
	scal[2] = 1e-4 * scale;
	...
	while (fgets(iline, BUFSIZ, lu) != NULL) {
		...
		case 'r':
			...
			if ((sscanf(iline, "r %d %d", &i, &n)) != 2)
				...
				(void)mk_reg(n);
				if (num_reg != i)
					ugh(line, "region sync error", ierr++);
			}
			break;
		...
		case 's':
			...
			for (i = 0; i < n_imp; i++) {
				...
				if (sscanf(ip, " %d", &(soltoimp[i])) != 1) {
					ugh(line, "less data than specified impurity number",
						ierr++);
					break;
				}
		...
		case 'I':
			...
			for (i = 0; i < n_imp; i++) {
				...
				if (n)
					SET_FLAGS(soltoimp[i], IMPLANTED_IMP);
				else
					CLEAR_FLAGS(soltoimp[i], IMPLANTED_IMP);
...
void ugh(int line, char *s, int ierr) {
	fprintf(stderr, "Input mesh error in line %d: %s\n", line, s);
}

/*-----------------IG2_WRITE--------------------------------------------
 * Write an IGGI-II format mesh.
 *----------------------------------------------------------------------*/
int ig2_write(char *name, int lflip, float scale) {
	FILE *lu;
	double scal[3];
	...
	for (ip = 0; ip < np; ip++) {
		fprintf(lu, "c %d", ip + 1);
		for (i = 0; i < mode; i++)
			fprintf(lu, " %g", cordinate(ip, i) * scal[i]);
		fprintf(lu, "  0\n");
	}
...
#define tcode(ie, j)                                                           \
	((neigh_t(ie, j) >= 0) ? neigh_t(ie, j) + 1 : neigh_t(ie, j))
	...
	for (is = 0; is < n_imp; is++)
		fprintf(lu, " %d", soltoimp[is]);
	fprintf(lu, "\n");

	/* print the node data for all the nodes */
	for (in = 0; in < nn; in++) {
		fprintf(lu, "n %d   %d  ", pt_nd(in), mat_nd(in));
		for (is = 0; is < num_sol(in); is++)
			fprintf(lu, " %e", sol_nd(in, is));
		fprintf(lu, "\n");
	}

	fprintf(lu, "I %d  ", n_imp);
	for (i = 0; i < n_imp; i++)
		fprintf(lu, " %d", IS_IMPLANTED(soltoimp[i]));
	fprintf(lu, "\n");
