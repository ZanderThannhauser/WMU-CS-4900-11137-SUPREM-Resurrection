...
#include "./src/suprem/mesh/newbackside.h"
#include "./src/suprem/mesh/pi_meshio.h"
...
void structure(char *par, struct par_str *param) {
    ...
    if (get_bool(param, "mirror")) {
        ...
        if (get_bool(param, "left"))
            sign = -1;
        if (is_specified(param, "top") && get_bool(param, "top")) {
            xy = 1;
            sign = -1;
        }
...
int skel_write(char *str, int i1, float f1) { return (0); }
int pif_write(char *str) { return (0); }
int polygon_etch(char *str, int foo) { return (0); }
