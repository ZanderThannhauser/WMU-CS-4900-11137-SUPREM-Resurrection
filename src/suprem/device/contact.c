 ...
 *									*
 ************************************************************************/
int contact(char *par, struct par_str *param) {
	char *str;
	int num = -1;
	...
	if (is_specified(param, "generate"))
		gen_contact();
	...
	for (i = 0; i < ne; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			if (IS_CONT(i, j)) {
				...
				if (nt == -1)
					panic("consistency error in contact generation");
			...
			if (IS_CONT(ie, j)) {
				...
				if (contacts[n_con].np == (*len - 1)) {
					*len += 20;
					...
					contacts[n_con].ndc =
						sralloc(int, *len, contacts[n_con].ndc);
				}
