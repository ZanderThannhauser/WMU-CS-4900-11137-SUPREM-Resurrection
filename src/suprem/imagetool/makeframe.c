...
int makeframe(float *data, int NX, int NY, int NXFAC, int NYFAC, char *name,
    ...
    int iaddr, iaddr2, ixstart, iystart, ixstop, iystop, n, p;
    unsigned char *array;
    char *filename;
    float OXIDE_VALUE;
    float NITRIDE_VALUE;
                        ...
                        if (mode) {
                            if (fabs((double)fvalue) < CLIPVALUE)
                                ...
                                if (fvalue < 0)
                                    value =
                                        OFFSET +
                                        (int)(((max - fvalue - 28.0) * width) /
                                              (max - min - 28.0));
                                else
                                    value = OFFSET +
                                            (int)(((max - fvalue) * width) /
                                                  (max - min - 28.0));
                            }
                        } else
                            value = (int)(OFFSET + ((fvalue - min) * width) /
                                                       (max - min));
...
int mysmooth(int value) {
    ...
    quotient =
        ((float)remainder < (float)CRITERION / 2.0) ? quotient : quotient + 1;
