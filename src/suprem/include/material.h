...
#ifdef STATIC_ALLOCATION_TIME
	= {"/gas",		  "/oxide",	"/nitride",	 "/silicon", "/poly",
	   "/oxynitride", "/aluminum", "/photoresist", "/gaas"}
#endif
;
...
EXTERN double xcrystal[3]
					  [3]; /* transformation matrix from x,y to crstal cords */
...
EXTERN int nclt, nclp; /* how many chlorine pressure/temp entries*/

EXTERN float lbreak[Noxides], /* linear break points */
...
EXTERN float B_DealGrove, A_DealGrove;

/* Another gaggle of parameters for the erfc model */
EXTERN float tinitN, /* Initial nitride thickness */
	erf1_q, erf1_delta;
EXTERN char *erf_Lbb, *erf_H;
...
EXTERN double mu0[MAXMAT + 1], mue[MAXMAT + 1], E[MAXMAT], pr[MAXMAT],
	nux[MAXMAT];
EXTERN char *atherm[MAXMAT];	 /* formula for the thermal expansion coeff */
EXTERN double IntStress[MAXMAT]; /* intrinsic stresses */
