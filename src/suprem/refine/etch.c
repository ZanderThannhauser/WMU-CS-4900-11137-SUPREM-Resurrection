...
#include "./src/suprem/refine/rate.h"
#include "./src/suprem/refine/string_etch.h"
...
#include "./src/suprem/refine/interp.h"
...
#define FETCHD(N, V, S, D)                                                     \
	if (is_specified(param, N))                                                \
		V = S * get_float(param, N);                                           \
	else                                                                       \
		V = D;

#define MOVE(A, B, C)                                                          \
	{                                                                          \
		pt[A]->cordo[0] = pt[A]->cord[0] = B;                                  \
		pt[A]->cordo[1] = pt[A]->cord[1] = C;                                  \
		inside[A] = TRUE;                                                      \
	}

#define DIS(p1, p2)                                                            \
	hypot(pt[p1]->cord[0] - pt[p2]->cord[0], pt[p1]->cord[1] - pt[p2]->cord[1])
...
void user_etch(char *par, struct par_str *param) {
	...
	if (CHOSEN("oxide"))
		mater = SiO2;
	if (CHOSEN("oxynitride"))
		mater = OxNi;
	if (CHOSEN("nitride"))
		mater = SiNi;
	if (CHOSEN("photoresist"))
		mater = PhRs;
	if (CHOSEN("aluminum"))
		mater = Al;
	if (CHOSEN("gaas"))
	...
	for (i = 1; i < nn; i++) {
		if (xmi > cordinate(pt_nd(i), 0))
			xmi = cordinate(pt_nd(i), 0);
		if (xma < cordinate(pt_nd(i), 0))
			xma = cordinate(pt_nd(i), 0);
		if (ymi > cordinate(pt_nd(i), 1))
			ymi = cordinate(pt_nd(i), 1);
	...
	if (CHOSEN("left") || CHOSEN("right")) {
		...
		HERE; /* 2020: Example 5: happens first */
		...
		HERE; /* 2020: Example 5: happens second */
		...
		/* 2020: 'p' is declared as static, and other runs may set 'p[3]' to */
		/* something, and that space shouldn't be used here, so: */
		if (p[3])
			free(p[3]), p[3] = NULL;
...
#define min2(A, B) (A < B) ? A : B
		...
		for (i = 1, bp = sreg[ir]->bnd; i || bp != sreg[ir]->bnd;
			 i = 0, bp = bp->next) {
			if (num_tri_nd(nB(bp)) == 0)
				rem_1nd(nB(bp));
		}
		if (mode == ONED) {
			bp = bp->prev;
			if (num_tri_nd(nF(bp)) == 0)
				rem_1nd(nF(bp));
		}
