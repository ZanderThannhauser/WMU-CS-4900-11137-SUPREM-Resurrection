	...
	while (!done) {
		...
		for (i = 0; (i < num_edge_nd(n)) && !fnd; i++) {
			...
			if ((e != el) && (e != e1) && is_border(e)) {
				...
				for (j = 0; j < num_ele_edg(e) && reg_tri(tri_edg(e, j)) != r;
					 j++)
					;
...
int sub_2dskel(int s1, int s2, int *s, int *inside) {
	...
	for (g = 1, bp = bnd; g || bp != bnd; g = 0, bp = bp->next) {
		...
		verpv(bp->iscc);
		verpv(bp->edge);
		verpv(edg[bp->edge]->nd[0]);
		verpv(edg[bp->edge]->nd[1]);
		...
		if ((inside[nB(bp)] != OUT) && (inside[nF(bp)] == OUT)) {
			...
			/*walk around the existing region until we cross again*/
			for (bq = bp->next; (inside[nF(bq)] == OUT); bq = bq->next) {
				e = bq->edge;
				set(edg[e], MARKED);
				ad_edge(s[num], e, sreg[s[num]]->bnd, MAYBE, BEFORE);
			}
	...
	for (bp = bnd, g = 1; g || bp != bnd; g = 0, bp = bp->next) {
		...
		if (mode == TWOD) {
			if (lil(p, dp, q, dq, alph) == 0) {
				...
				if ((alph[0] >= 0.0) && (alph[0] <= 1.0) && (alph[1] >= 0.0) &&
					(alph[1] <= 1.0)) {
					c[0] = alph[0] * dp[0] + p[0];
					c[1] = alph[0] * dp[1] + p[1];
					return (bp);
