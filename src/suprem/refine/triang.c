...
void triang(int ir, int rbn, int bflag, int spflag) {
    ...
    if ((sreg[nsreg - 1]->len > 5) && bflag) {
        nr = nsreg - 1;
        for (f = 1, bp = sreg[nr]->bnd; (bp != sreg[nr]->bnd) || f;
             f = 0, bp = bp->next)
            if (ask(edg[bp->edge], ESURF))
                div_edg(bp->edge);
    }
...
void ck_clock(int ir, int fl) {
    ...
    if (debug2)
        printf("Region %d has angle %24.15lf\n", ir, tang);
    ...
    /* Hopeless if zero. */
    else if (dabs(tang) < EPS)
        panic("Region crosses itself.");
    ...
    /* Huh? */
        ...
        panic("Region has malformed boundary");
...
void free_skel(int nr) {
    ...
    for (r = 0; r < nreg; r++)
        if (reg[r]->sreg != -1)
            reg[r]->sreg = new_sreg[reg[r]->sreg];
