...
#include "./src/suprem/refine/offset.h"
#include "./src/suprem/refine/string_off.h"
...
#define chosen(A) (is_specified(param, A) && get_bool(param, A))
    ...
    if (get_bool(param, "arsenic"))
        ...
        imp = B;
    else if (get_bool(param, "gallium"))
        imp = Ga;
    ...
    iskel =
        build_skel(nump, p, numop, op, ndiv, r, (double)thick / ndiv, square);
...
#define min2(A, B) ((A < B) ? (A) : (B))
    ...
    for (i = 0; i < nump; i++) {
        ...
        if (ln != -1) {
            ...
            for (ef = -1, e1 = 0; e1 < num_edge_nd(ln) && ef == -1; e1++) {
                e = edge_nd(ln, e1);
                if ((nd_edg(e, 0) == node) || (nd_edg(e, 1) == node))
                    ef = e;
            }
            if (ef == -1)
                ef = mk_edg(ln, node);
    ...
    for (i = 0; i < div - 1; i++) {
        ...
        c[0] =
            p[nump - 1].x + (op[numop - 1].x - p[nump - 1].x) * (i + 1) / div;
        c[1] =
            p[nump - 1].y + (op[numop - 1].y - p[nump - 1].y) * (i + 1) / div;
