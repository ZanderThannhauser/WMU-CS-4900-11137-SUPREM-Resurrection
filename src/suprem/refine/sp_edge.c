...
int upd_edg(int ie, int in) {
	...
	for (i = 0; i < num_ele_edg(ie); i++)
		add_list(&tneigh, ele_edg(ie, i));

	for (i = 0; i < tneigh.num; i++)
		new_edg_tri(ie, enew, tneigh.list[i]);
	dis_list(&tneigh);

	/*build a neighbor list*/
	for (i = 0; i < num_skel_edg(ie); i++)
		add_list(&tneigh, skel_edg(ie, i));

	for (i = 0; i < tneigh.num; i++)
		new_edg_skel(ie, enew, tneigh.list[i]);
	dis_list(&tneigh);
...
int insert_pt_edg(int e, float c[MAXDIM], int *p) {
	...
	*p = ip = mk_pt(2, c);
	...
	for (i = 0; i < num_sol(in); i++)
		set_sol_nd(in, i,
				   rat * sol_nd(nd_edg(e, 1), i) +
					   irat * sol_nd(nd_edg(e, 0), i));
	...
	if (eoth != -1) {
		...
		/*interpolate the solutions*/
		rat = pdist(ip, pt_edg(eoth, 0)) /
			  pdist(pt_edg(eoth, 0), pt_edg(eoth, 1));
		irat = 1.0 - rat;
		for (i = 0; i < num_sol(in); i++)
			set_sol_nd(in, i,
					   rat * sol_nd(nd_edg(eoth, 1), i) +
						   irat * sol_nd(nd_edg(eoth, 0), i));
	}
...
void new_edg_skel(int ie, int enew, int ir) {
	...
	lep = eindex(ir, ie);
	if (lep == NULL)
		panic("internal error: edge not in region (sp_edge)");
...
void new_edg_tri(int e, int enew, int it) {
	...
	if (mode == ONED) {
		...
		n[1] = nshare;
		n[2] =
			(nd_edg(enew, 1) == nshare) ? (nd_edg(enew, 0)) : (nd_edg(enew, 1));
	...
	for (i = 0; i < num_edge(it); i++)
		sub_ele_edg(edg_ele(it, i), it);
	fing_tri(it);
