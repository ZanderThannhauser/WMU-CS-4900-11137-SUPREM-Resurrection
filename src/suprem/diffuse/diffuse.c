...
#include "./src/suprem/diffuse/predep.h"
...
void diffuse(char *par, struct par_str *param) {
    ...
    if (get_bool(param, "argon"))
        gas_type = GAS_ARGON;
    if (get_bool(param, "nitrogen"))
        gas_type = GAS_ARGON;
    if (get_bool(param, "ammonia"))
        gas_type = GAS_ARGON;
    if (get_bool(param, "dryo2"))
        gas_type = GAS_DRYO2;
    if (get_bool(param, "weto2"))
        gas_type = GAS_WETO2;
    if (get_bool(param, "antimony"))
        gas_type = GAS_Sb;
    if (get_bool(param, "arsenic"))
        gas_type = GAS_As;
    if (get_bool(param, "boron"))
        gas_type = GAS_B;
    if (get_bool(param, "phosphorus"))
        gas_type = GAS_P;
    if (get_bool(param, "beryllium"))
        gas_type = GAS_Be;
    if (get_bool(param, "magnesium"))
        gas_type = GAS_Mg;
    if (get_bool(param, "selenium"))
        gas_type = GAS_Se;
    if (get_bool(param, "isilicon"))
        gas_type = GAS_Si;
    if (get_bool(param, "tin"))
        gas_type = GAS_Sn;
    if (get_bool(param, "germanium"))
        gas_type = GAS_Ge;
    if (get_bool(param, "zinc"))
        gas_type = GAS_Zn;
    if (get_bool(param, "carbon"))
        gas_type = GAS_C;
    if (get_bool(param, "generic"))
        gas_type = GAS_G;
    ...
    times(&before);
    solve_time(time, temp, methdata.init_time, methdata.timemeth, dump, movie,
               cont);
    times(&after);
