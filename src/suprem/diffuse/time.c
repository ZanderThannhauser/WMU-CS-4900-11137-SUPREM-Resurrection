...
double timed_trbdf(double delta, float temp, int compute) {
    ...
    /*load the values*/
    for (j = 0; j < n_imp; j++) {
        if ((!IS_DIFFUSE(soltoimp[j])) && (!IS_PSEUDO(soltoimp[j])))
            continue;
        if ((IS_STEADY(soltoimp[j])) && (!IS_LOCKED(soltoimp[j])))
            continue;
        for (i = 0; i < nn; i++)
            new[j][i] = mid[j][i] = old[j][i] = nd[i]->sol[j];
    }
    ...
    if (compute) {
        ...
        if (xv != -1) {
            /*update the grid locations*/
            i = soldif_tr(delta, temp, old, mid, olda, mida, oarea, narea,
                          TRUE);
        } else {
            i = soldif_tr(delta, temp, old, mid, olda, mida, oarea, oarea,
                          TRUE);
        }
    ...
    /*total the areas up*/
    if ((verbose >= V_CHAT) && (xv != -1))
        tot_areas(oarea, narea);
