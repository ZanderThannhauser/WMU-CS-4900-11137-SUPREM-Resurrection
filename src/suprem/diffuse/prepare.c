...
#include "./src/suprem/math/genrow.h"
...
#define Free(P)                                                                \
	...
	P = 0;
#define NO_COMPUTE(A)                                                          \
	((A == XVEL) || (A == YVEL) || (A == DELA) || (A == Sxx) || (A == Sxy) ||  \
	 (A == Syy))
...
void upd_matrix() {
	...
	for (i = 0; i < ss_nsol; i++) {
		for (j = 0; j < ss_nsol; j++)
			facblk[ss_sol[i]][ss_sol[j]] = FALSE;
	...
	for (i = 0; i < lnsol; i++) {
		if (methdata.fill == FULL)
			for (j = 0; j < lnsol; j++)
				facblk[lsol[i]][lsol[j]] = TRUE;
		else if (methdata.fill == DIAG)
			for (j = 0; j < lnsol; j++)
				facblk[lsol[i]][lsol[j]] = FALSE;
...
void diff_blocks() {
	...
	for (i = 0; i < tm_nsol; i++) {
		...
		for (tstelm = TRUE, j = 0; j < tm_nsol; j++) {
			sj = tm_sol[j];
			tstelm = tstelm && ((blktype[si][sj] == B_DIAG) ||
								(blktype[si][sj] == B_NONE));
		}
		...
		if (tm_elim[i]) {
			...
			for (j = 0; j < tm_nsol; j++) {
				...
				for (k = 0; k < tm_nsol; k++) {
					...
					if ((blktype[sj][si] == B_DIAG) &&
						(blktype[sj][sk] == B_NONE)) {
		...
		for (j = ia[i]; j < ia[i + 1]; j++) {
			...
			if (nd[i]->pt == nd[ia[j]]->pt) {
				...
				for (k = 0; nx1 == -1 && k < num_edge_nd(i); k++) {
					m = edge_nd(i, k);
					if ((nd_edg(m, 0) == ia[j]) || (nd_edg(m, 1) == ia[j]))
						nx1 = m;
				}
				edg[nx1]->l = j;
				edg[nx1]->u = j + aoff;
