...
#include "./src/suprem/include/material.h" /* for the substrate orientation */
...
#include "./src/suprem/diffuse/movie.h"
	...
	while (t < time) {
		...
		if (verbose >= V_NORMAL)
			...
			printf("Solving %12g + %12g = %12g, %8g%%, np %d\n", total, del_df,
				   total + del_df, 100 * del_df / last_ddf, np);
...
double solstep(double del_df, float temp, int timemeth, int compute) {
	...
	if (n_imp != 0) {
		switch (timemeth) {
		case TRBDF:
			Kt = timed_trbdf(del_df, temp, compute);
			...
			break;
		case FORM:
			Kt = timed_trbdf(del_df, temp, FALSE);
			break;
		}
	}
	...
	if (Kt < 0.0) {
		...
		if (xv != -1) {
			for (i = 0; i < nn; i++) {
				...
				mid[dela][i] =
					(1.0 - scale) * old[dela][i] + scale * new[dela][i];
			}
		}
		...
		for (j = 0; j < tm_nsol; j++) {
			sj = tm_sol[j];
			for (i = 0; i < nn; i++)
				new[sj][i] = mid[sj][i] = old[sj][i] = nd[i]->sol[sj];
		}
...
float time_form(char *expr, float total, float dt) {
	...
	if ((s = index(expr, 'g')) != 0 && s[1] == 'd' && s[2] == 't') {
		...
		if (gdt != 0)
			...
			fprintf(stderr,
					"*** Sorry, gdt is zero, sticking with timestep = %g\n",
					dt);
			return (dt);
		}
	}
...
double start_step(float temp, int cont) {
	...
	for (j = 0; j < n_imp; j++) {
		...
		if (cimp > 0)
			for (i = 0; i < nn; i++)
				nd[i]->sol[j] = olda[imptosol[cimp]][i];
	}
	...
	if (oxidizing) {
		...
		if (oxide_model == O_ELASTIC || oxide_model == O_VISCOUS ||
...
double grid_dt() {
	 ...
	 */
	if (grain_growing)
		spacing = grain_grid * grain_gdt;
	else
		spacing = oxide_grid * oxide_gdt;
	if (spacing <= 0) {

		/* This one isn't supposed to happen */
		if (grain_gdt <= 0) {
			fprintf(stderr,
					"*** The parameter grain.gdt on the method card is zero, "
					"choosing 0.25!\n");
			grain_gdt = 0.25;
		} else if (oxide_gdt <= 0) {
			fprintf(stderr,
					"*** The parameter oxide.gdt on the method card is zero, "
					"choosing 0.25!\n");
			oxide_gdt = 0.25;
		}
