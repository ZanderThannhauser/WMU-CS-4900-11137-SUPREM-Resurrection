...
#else
        for (i = 0; i < nn; i++)
            equil[bs][i] = dequ[bs][i] = net[i] = dnet[bs][i] = rhs[bs][i] =
                newb[bs][i] = cs->newa[bs][i] = 0.0;
#endif
        for (j = 0; j < cs->nsol; j++) {
            js = cs->sol[j];
...
void time_val(int sol, double *rhsnm, struct call_str *cs) {
    ...
    if (cs->type == TR) {
        ...
        for (i = 0; i < nn; i++) {
            ...
            /*the time terms*/
            rhs[sol][i] -= cpl * (cs->nco[i] * cs->new[sol][i] -
                                  cs->oco[i] * cs->old[sol][i]);
...
void setup_2d(int sol, float temp, int nsol, int *solve, double *inter,
    ...
    if (impur[imp].block_set != NULL)
        impur[imp].block_set(sol, temp, nsol, solve, newarea, chem, act, inter,
                             vacan);

    /*if no boundary conditions, we are done*/
    if (impur[imp].boundary == NULL)
        return;
    if (IS_PSEUDO(imp))
        return;
