...
int flip() {
	...
	if (!((todo = scalloc(int, 1 + ned)) && (done = scalloc(int, 1 + ned))))
		return (-1);
	...
	do {
		...
		for (it = 0; it < ned; it++) {
			...
			if (reg_tri(tri_edg(it, 0)) != reg_tri(tri_edg(it, 1)))
				continue;
...
void tri_flip(int it, int tp, int tq) {
	...
	for (tr = tp; tr == tp || tr == tq;
		 tr += tq - tp) { /* for each of tp, tq */
		if ((fixme = tri[tr]->nb[0]) < 0)
			continue;
		for (i = 0; i < 3; i++)
			if (tri[fixme]->nb[i] == tp + tq - tr)
				tri[fixme]->nb[i] = tr;
	}
