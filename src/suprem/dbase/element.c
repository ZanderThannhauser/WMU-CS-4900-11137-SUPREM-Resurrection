...
char *alloc_tri() {
    ...
    if (!tri[ne])
        return ("Out of storage in alloc_tri");
...
int mk_ele(int nv, int *v, int nb, int *b, int r, int flag) {
    ...
    for (j = 0; j < num_face(t); j++) {
        if (is_face_bck(t, j)) {
            ...
            if (flag)
                for (p = 0; p < i; p++)
                    set_back(nd_face_ele(t, j, p));
            else
                for (p = 0; p < i; p++)
                    set_back(pt_nd(nd_face_ele(t, j, p)));
        }
        if (is_face_exp(t, j)) {
            i = num_nd_fc(t, j);
            if (flag)
                for (p = 0; p < i; p++)
                    set_surf(nd_face_ele(t, j, p));
            else
                for (p = 0; p < i; p++)
                    set_surf(pt_nd(nd_face_ele(t, j, p)));
        }
    }
    return (ne - 1);
}
 ...
 */
int mk_ele_pt(int nv, int *v, int reg) {
    char *err;
    int i;
 ...
 */
void dis_1tri(struct tri_str **t) {
    /*free the structure*/
    free(t[0]);
