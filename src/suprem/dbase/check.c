...
void mtest1(char *when) {
    ...
    for (i = 0; i < nreg; i++)
        if (mat_reg(i) != Si && mat_reg(i) != Poly && mat_reg(i) != SiNi &&
            mat_reg(i) != SiO2 && mat_reg(i) != OxNi && mat_reg(i) != PhRs &&
            ...
            report("region %d has bad material code %d\n", i, mat_reg(i))
                ...
                if (is_surf(p) || is_back(p)) {
                    if (!gassy)
                        report("exposed point %d has no gas node\n", p)
                } else if (gassy)
                    report("unexposed point %d has gas node\n", p)
            }
        ...
        if (mat_nd(n) == GAS) {
            ...
            if (num_edge_nd(n) == 0)
                report("material node %d has no linked edges\n", n)
                    ...
                    else for (i = 0; i < num_edge_nd(n); i++) {
                    t = edge_nd(n, i);
                    if (t < 0 || t >= ned)
            ...
            if (tn >= 0) {
                if (tn == t)
                    report("triangle %d is its own %d neighbor\n", t,
                           i) else if (!tnabor(t, tn, &n, &p))
                        report("connection %d of triangle %d is wrong\n", i,
                               t) for (j = 0; j < num_face(tn);
                                       j++) if (neigh_fc(tn, j) == t) break;
                if (j == num_face(tn))
