...
#include "./src/suprem/include/diffuse.h" /* for noni */
...
int FEconvert(int element, int dof) {
	...
	for (ie = 0; ie < FEne; ie++) {
		...
		for (j = 0; j < 3; j++) {
			...
			if (CylindricalSymmetry) {
				...
				if ((nbj < 0 && nbj > BC_OFFSET) ||
					...
					if (rad < 0.999 * rad2 || rad2 < 0.999 * rad)
						panic("huh?");
		...
		/*bubble node*/
		if (FEdesc[element].nel < 7)
			continue;

		FEelt[ie]->nd[6] = FEnn;
		FEnd[FEnn]->cord[0] = (FEnd[nl[0]]->cord[0] + FEnd[nl[1]]->cord[0] +
							   FEnd[nl[2]]->cord[0]) /
							  3;
		FEnd[FEnn]->cord[1] = (FEnd[nl[0]]->cord[1] + FEnd[nl[1]]->cord[1] +
							   FEnd[nl[2]]->cord[1]) /
							  3;
		FEnn++;
	}
 ...
 *----------------------------------------------------------------------*/
void dummy_sil(int doSil) /* tru => simulate silicon */
{
	int ie, i, j;
	FEelt_typ *ae;
	FEnd_typ *an;
	...
	for (ie = 0; ie < FEne; ie++) {
		...
		/* Skip if non-dummy */
		if ((doSil && ae->mat == Si) || (!doSil && ae->mat != Si))
			continue;
...
#define SavNoSil 1
#define SavOnlySil 2
void FE2s4all() { FE2s4(SavAll); }
void FE2s4ox() { FE2s4(SavNoSil); }
void FE2s4sil() { FE2s4(SavOnlySil); }
...
void FE2s4(int SavWhat) {
	...
	for (i = 0; i < 5; i++)
		add_impurity(imps[i], 0.0, -1);
	for (in = 0; in < nn; in++)
		for (i = 0; i < 5; i++)
			nd[in]->sol[imptosol[imps[i]]] = 0;
	...
	for (ip = 0; ip < np; ip++) {
		...
		for (j = 0; j < ap->nn; j++) {
			...
			/* Copy stresses */
			if (SavWhat == SavAll ||
				(SavWhat == SavOnlySil && ja->mater == Si) ||
