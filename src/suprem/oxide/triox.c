 ...
 *									*
 ************************************************************************/
void triox_shape(s, t, Cord, xsj, shp) double s, t; /* natural coordinates*/
...
#include "./src/suprem/include/FEgeom.h" /* for MAXDOF */
#include "./src/suprem/include/FEmath.h" /* for lambda, noderiv */
#include "./src/suprem/include/global.h" /* for SAT */
        ...
        if (do_sdep) {
            ...
            for (c = 0; c < NEL; c++) {
                shpx[c][X] = (l >= 3)
                                 ? shpg[l - 3][c][X]
                                 : shpg[(l + 1) % 3][c][X] +
                                       shpg[(l + 2) % 3][c][X] - shpg[l][c][X];
                shpx[c][Y] = (l >= 3)
                                 ? shpg[l - 3][c][Y]
                                 : shpg[(l + 1) % 3][c][Y] +
                                       shpg[(l + 2) % 3][c][Y] - shpg[l][c][Y];
            }
 ...
 *									*
 ************************************************************************/
void triox_stress(mat, coeff, xl, ul, where, eps, sig, xsj) int mat;
int (*coeff)();
double xl[NEL][2];
double ul[NEL][3];
double *where;
...
void TrioxBCinit() {
     ...
     */
    for (in = 0; in < FEnn; in++) {
        if (FEnd[in]->fixity[X] != 4 && FEnd[in]->fixity[Y] != 4)
            continue;
    ...
    for (ie = 0; ie < FEne; ie++) {
        ...
        for (j = 0; j < 3; j++) {
            ...
            if (nbr >= 0 && FEelt[nbr]->mat == Si) {
                ...
                /*Normal points into oxide*/
                ip = pie->nd;
                nx = FEnd[ip[(j + 1) % 3]]->cord[1] -
                     FEnd[ip[(j + 2) % 3]]->cord[1];
                ny = FEnd[ip[(j + 2) % 3]]->cord[0] -
                     FEnd[ip[(j + 1) % 3]]->cord[0];
                len = hypot(nx, ny);
...
test_tbc(ie, iv, vx, vy) int ie; /* Which triangle to work on */
...
#define MELDOF 60
    ...
    (*(desc->bound))(stiff, wrhs, xl, disp, desc->coeff, pie->mat, fixity, bc,
                     ie);

    disp[FEdf * iv + X] += vx;
    disp[FEdf * iv + Y] += vy;

    (*(desc->bound))(stiff1, wrhs1, xl, disp, desc->coeff, pie->mat, fixity, bc,
                     ie);

    for (i = 0; i < nel; i++) {
        for (j = 0; j < FEdf; j++) {
            printf("%2d %2d %12.5g %12.5g\n", i, j,
                   wrhs1[FEdf * i + j] - wrhs[FEdf * i + j],
...
test_jacob(ia, aoff, in, iv, v, il, loff) int *ia, aoff; /* matrix map */
...
{
    int neq, i, jn, jv, jf;
    double *a, *a1, *rhs, *rhs1, *deltav, *drhs;
    double *l;
...
test_stress(n) int n;
{
    ...
    for (ie = 0; ie < FEne; ie++) {
        for (iv = 0; iv < 3; iv++) {
            ...
            if (pie->mat != SiO2 || pie->face[iv] < 0 ||
                FEelt[pie->face[iv]]->mat != Si || n <= 1)
                continue;
            ...
            for (f = 0; f < 1.0; f += 1.0 / n) {
                ...
                printf("[%8f,%8f] eps(%12.5g, %12.5g, %12.5g)\n", where[0],
                       where[1], eps[XX], eps[YY], eps[XY]);
            }
#endif /*notdef*/
            (*(desc->nodal_stress))(xl, disp, epsn, sign, desc->coeff,
                                    pie->mat);
            ...
            for (f = 0.5 * (1 - r3i); f < 1; f += r3i) {
                ...
                area[iv1] = f;
                area[iv2] = 1 - f;
                x = (area[iv] * xl[2 * iv + X] + area[iv1] * xl[2 * iv1 + X] +
                     area[iv2] * xl[2 * iv2 + X]);
                y = (area[iv] * xl[2 * iv + Y] + area[iv1] * xl[2 * iv1 + Y] +
                     area[iv2] * xl[2 * iv2 + Y]);
                where[0] = area[1];
                where[1] = area[2];
        ...
        for (j = 0; j < 3; j++) {
            if (FEelt[i]->face[j] == -1022) {
                ...
                printf("(%g,%g) l(%g,%g,%g) c(%g,%g,%g) r(%g,%g,%g)\n", dx, dy,
                       FDrhs[3 * k1 + X], FDrhs[3 * k1 + Y],
                       ffcos(dx, dy, FDrhs[3 * k1 + X], FDrhs[3 * k1 + Y]),
                       FDrhs[3 * k + X], FDrhs[3 * k + Y],
                       ffcos(dx, dy, FDrhs[3 * k + X], FDrhs[3 * k + Y]),
                       FDrhs[3 * k2 + X], FDrhs[3 * k2 + Y],
                       ffcos(dx, dy, FDrhs[3 * k2 + X], FDrhs[3 * k2 + Y]));
            }
        }
    }
}
double ffcos(ax, ay, bx, by) double ax, ay, bx, by;
{
    double t1, t2, t3;
    t1 = ax * bx + ay * by;
    t2 = ax * ax + ay * ay;
