...
void c_mater(char *par, struct par_str *param) {
	...
	if (mat == SiO2) {
		if (is_specified(param, "wet") && get_bool(param, "wet"))
			wd = 1;
		if (is_specified(param, "dry") && get_bool(param, "dry"))
			wd = 0;
		if (((is_specified(param, "visc.0")) ||
	...
	if (is_specified(param, "n.type") && get_bool(param, "n.type"))
		type = 0;
	if (is_specified(param, "p.type") && get_bool(param, "p.type"))
		type = 1;
...
#define cross3(A, B, C)                                                        \
	...
	default:
		fprintf(stderr,
				"Warning: unknown substrate orientation, using <100>\n");
		xcrystal[1][X] = 1;
		xcrystal[1][Y] = 0;
		xcrystal[1][Z] = 0;
		break;
...
void SupToXtal(double *normal, double *vec3) {
	...
	if (mode == TWOD) {
		...
		if (normal && ornt_dep &&
			(nlen = hypot(normal[0], normal[1])) > 1e-30) {
