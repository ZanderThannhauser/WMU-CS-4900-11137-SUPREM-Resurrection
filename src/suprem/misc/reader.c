...
int read_list(struct par_str **par, FILE *fd) {
    ...
    while (TRUE) {
        ...
        if (temp == level) {
            ...
            level++;
            par[0]->param =
                (struct par_str **)calloc(NUMPAR + 1, sizeof(int *));
...
int read_param(struct par_str *par, FILE *fd) {
    ...
    case REAL:
        if ((eof = fread(&par->def.dval, sizeof(float), 1, fd) != 1))
            return (eof);
        break;
    case COMM:
    case BOOL:
    case CHOICE:
    case INT:
        if ((eof = fread(&par->def.ival, sizeof(int), 1, fd) != 1))
            return (eof);
        break;
    case STR:
        for (c = 'a', i = 0; c != '\0'; i++) {
...
int read_boolean(struct bool_exp **bexp, FILE *fd) {
    ...
    case OPER: /*write out the four byte descriptor*/
        if ((eof = fread(&(bexp[0]->value.ival), sizeof(int), 1, fd)) != 1)
            return (eof);
        break;
    case REAL: /*write out the real number*/
        if ((eof = fread(&(bexp[0]->value.dval), sizeof(float), 1, fd)) != 1)
            return (eof);
        break;
