...
#include "./src/suprem/diffuse/active.h"
...
char *get_solval(float *val, int type) {
    ...
    case QFN:
        ...
        isol = imptosol[N];
        for (i = 0; i < nn; i++)
            val[i] = Phi_n(sol_nd(i, isol), sol_nd(i, psol), Ec[i], mat_nd(i),
                           &d1, &d2);
        break;
    ...
    case QFP:
        ...
        isol = imptosol[H];
        for (i = 0; i < nn; i++)
            val[i] = Phi_p(sol_nd(i, isol), sol_nd(i, psol), Ev[i], mat_nd(i),
                           &d1, &d2);
        break;
    ...
    case DATA:
        ...
        if (lex == NULL) {
            if (rex->type != STRING) {
                fprintf(
                    stderr,
                    "error in data function, argument must be a filename\n");
            } else {
            ...
            if (lex->type != STRING) {
                ...
                if (read_data(val, lex->value.sval, (int)rex->value.dval, 0) ==
                    -1)
                    fprintf(stderr, "error in data function, empty file");
            }
        }
