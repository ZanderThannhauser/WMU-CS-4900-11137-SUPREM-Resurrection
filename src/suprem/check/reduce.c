 ...
 *									*
 ************************************************************************/
void expr_reduce(a, b, c, d) struct tok_str *a, *b, *c, *d;
{
    struct vec_str *tmp;
...
int reduce() {
    ...
    case EXPR: /*look back at previous stuff to decide*/
        ...
        case OP3: /*if preceeding operator is an expr, reduce*/
            if (prev->type == EXPR) {
                ...
                expr_reduce(prev, top, &input_token, &input_token);
                st_pnt -= 2;
                return (TRUE);
            }
            break;
    ...
    case OP2: /*check back on the stack for this stuff*/
        if ((top->type == EXPR) && (prev->type == OP2) && (old->type == EXPR)) {
