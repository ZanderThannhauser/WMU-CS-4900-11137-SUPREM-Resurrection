...
val_str bool_check(struct bool_exp *bexp, int *typ) {
    ...
    case OPER: /*switch dependent on the operator type*/
        ...
        case '*':
            ...
            break;
        case '|':
            temp.ival = left.ival || right.ival;
            break;
        case '&':
            temp.ival = left.ival && right.ival;
            break;
    ...
    case PARVAL: /*return the parameter value*/
        ...
        break;
    default:
        panic("What type am I supposed to check???\n");
        break;
    }
    return (temp);
}
...
int error_check(struct par_str **par, int bool) {
    ...
    for (; par[0] != NULL; par++) {
        ...
        if ((par[0]->param != NULL) && !(par[0]->type & CHOICE) && !err_ret) {
            if ((err_ret = error_check(par[0]->param, par[0]->value.ival)) <
                0) {
                if (err_ret == -2)
                    fprintf(
                        stderr,
                        "%s has to be true for sub parameters to be legal\n",
                        par[0]->name);
                err_ret = -1;
            }
        }
