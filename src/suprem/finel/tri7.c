 ...
 *									*
 ************************************************************************/
void tri7_shape(s, t, cord, xsj, shp) double s, t; /* natural coordinates*/
                     ...
                     shp[i][0/1] = x,y derivative of shp[i][2] */
{
    ...
    shp[6][0] =
        9 * (lam1 * lam2 * (-1) + lam0 * lam2 * (1) + lam0 * lam1 * (0));
    shp[6][1] =
        9 * (lam1 * lam2 * (-1) + lam0 * lam2 * (0) + lam0 * lam1 * (1));
 ...
 *									*
 ************************************************************************/
void tri7_stress(mat, coeff, xl, ul, where, eps, sig, xsj) int mat;
int (*coeff)();
double xl[7][2];
double ul[7][2];
double *where;
...
void tri7_nodal_stress(double xl[7][2], double ul[7][2], double epsn[7][3],
                       double sign[7][3], int (*coeff)(), int mat,
                       double *xsj) {
