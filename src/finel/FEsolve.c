...
void FEsolve(int verbose) {
    ...
    if (ContinMeth == 0)
        ...
        FEstep(ia, aoff, a, il, loff, l, rhs, solo, deln, delo, derr, 0.25,
               0.25);
        FEstep(ia, aoff, a, il, loff, l, rhs, solo, deln, delo, derr, 0.25,
               0.5);
...
void FEfdiff(int *ia, int aoff, double *a, int *il, int loff, double *l,
    ...
    do {
        ...
        if (dl < 1.5 * odl)
            VECTOR2 if (FEnd[i]->fixity[j] != 1) FEnd[i]->sol[j] =
                solo[k] + dl / odl * delo[k];
#endif
...
int FEnewton(int *ia, int aoff, double *a, int *il, int loff, double *l,
    ...
    static float CutCrit =
        1.0e-2; /* How far to keep cutting tk - 0.5 means give up */
    ...
    /* The Newton loop. */
    nGrow = 0;
    for (loop = 0, converge = 0; !converge && loop < FEnonloop;
         loop += noderiv ? 1 : 2) {
        ...
        while (tk > CutCrit) {
            ...
            if (FEverbose)
                printf("Newton loop %2d%c cut %8.2g upd %10.4g orhs %10.4g rhs "
                       "%10.4g\n",
                       loop, noderiv ? '*' : ' ', tk, upd, onorm, norm);

            /* Try rescuing a NR step */
            if (norm >= onorm && noderiv)
                break;
...
void FEsymb(int **Pia, int **Pil, int *Paoff, int *Ploff, int **Ppv) {
    ...
    if (lsize != 0)
        loff = FEdf * FEdf * FEnn + 1 + FEdf * FEdf * lsize;
    else
        loff = 80 * FEnn * FEdf * FEdf;
    il = salloc(int, loff);
 ...
 *----------------------------------------------------------------------*/
FEsymb1(Pia, Pil, Paoff, Ploff, Ppv) int **Pia, **Pil, *Paoff, *Ploff, **Ppv;
{
 ...
 *----------------------------------------------------------------------*/
void build_ia(int **pia) /* The list to generate */
{
    struct IntP {
        int val;
        struct IntP *next;
...
/*-----------------FEupdate---------------------------------------------
 *----------------------------------------------------------------------*/
FEupdate(delta, loop) double *delta;
int loop;
{
    int i, j;
...
#define loop                                                                   \
    ...
    case RESTORE:
        ...
        break;
    case UPD:
        loop if (FEnd[i]->fixity[j] != 1) FEnd[i]->sol[j] += v[i * FEdf + j];
        break;
    case DELV:
        loop default : panic("notreached");
    }
    return (0);
}
 ...
 *----------------------------------------------------------------------*/
mxvl(nv, ia, aoff, a, v, av, lvl) int nv;
int *ia;
int aoff;
double *a;
double *v;
 ...
 *----------------------------------------------------------------------*/
bubbleUp(Preorder) int **Preorder;
{
    int *reorder;
...
dump_A(ia, aoff, il, loff) int *ia, aoff, *il, loff;
{
    int lneq, i, jn, jv, jf, j, *ipvt;
    double *a, *a1, *rhs, *rhs1, *deltav, *drhs;
    double *l, *mega, *work, cond, diff;
    ...
    if (fwrite(a, sizeof(double), ia[lneq] - 1 + aoff, lu) !=
        ia[lneq] - 1 + aoff) {
    ...
    for (i = 0; i < lneq; i++) {
        diff =
            fabs((rhs[i] - work[i]) / (1e-60 + fabs(rhs[i]) + fabs(work[i])));
        if (diff > mdiff)
            mdiff = diff;
        if (diff > dtol)
            printf("%d %d %12.5g %12.5g\n", i / FEdf, i % FEdf, work[i],
                   rhs[i]);
    }
    ...
    printf("diff %g update norms 0 %g 1 %g 1f %g\n", mdiff, FEupdnorm(rhs),
           FEupdnorm(arhs), FEupdnorm(arhs2));
...
dump_A_convex() { dump_A(FDia, FDaoff, FDil, FDloff); }
#endif /* convex */
