...
void predep() {
    ...
    for (i = 0; i < nn; i++)
        if (nd[i]->mater == GAS)
            for (j = 0; j < n_imp; j++)
                if (IS_DIFFUSE(soltoimp[j]))
                    nd[i]->sol[j] = 0.1 * ABE[soltoimp[j]];

    /*worry about getting the gas right*/
    /*here are checks on all impurity types as they are added*/
    if ((gas_type != GAS_B) && (gas_type != GAS_As) && (gas_type != GAS_Sb) &&
        (gas_type != GAS_P) && (gas_type != GAS_DRYO2) &&
        (gas_type != GAS_WETO2) && (gas_type != GAS_Be) &&
        (gas_type != GAS_Mg) && (gas_type != GAS_Se) && (gas_type != GAS_Si) &&
        (gas_type != GAS_Sn) && (gas_type != GAS_Ge) && (gas_type != GAS_Zn) &&
        (gas_type != GAS_C) && (gas_type != GAS_G))
        return;
