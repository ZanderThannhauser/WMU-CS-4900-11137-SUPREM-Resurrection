 ...
 *									*
 ************************************************************************/
void Cdiff_coeff(float temp, double **new, double **equil,
 ...
 *									*
 ************************************************************************/
void Cactive(int simple, int nn, float temp,
...
void carbon(char *par, struct par_str* param)
{
    ...
    for (; i <= j; i++) {
        if (i == 0)
            CLEAR_FLAGS(iC, IMPLANTED_IMP);
        else
            SET_FLAGS(iC, IMPLANTED_IMP);
