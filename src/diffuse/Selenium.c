 ...
 *									*
 ************************************************************************/
double Sediff_coeff(temp, new, equil, noni, idf, vdf, iprt, vprt)
 ...
 *									*
 ************************************************************************/
Seactive( simple, nn, temp, conc, act, equil, noni )
int simple;
int nn;
float temp;
double **conc;
...
{ 
    ...
    for (; i <= j ; i++) {
	if ( i == 0 )	CLEAR_FLAGS(iSe, IMPLANTED_IMP);
	else		SET_FLAGS(iSe, IMPLANTED_IMP);
