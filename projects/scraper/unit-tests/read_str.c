...
void test(const char* str_file_content, struct str_data str_data_input){
    ...
    for (size_t i = 0; i < str_data_input.size; i++)
    {
       assert(actual_str_data.rows[i].depth_from_surface == str_data_input.rows[i].depth_from_surface);
       assert(actual_str_data.rows[i].boron_concentration == str_data_input.rows[i].boron_concentration);
       assert(actual_str_data.rows[i].phosphorus_concentration == str_data_input.rows[i].phosphorus_concentration);
       assert(fabs(actual_str_data.rows[i].net_doping_concentration - str_data_input.rows[i].net_doping_concentration) < .000001);
    }
