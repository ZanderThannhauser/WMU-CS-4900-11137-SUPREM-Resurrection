...
void Vboundary(struct bound_str *bval)
{
	...
	if (model(V, mat0, mat1) & GROWTH)
	{
		...
		tmp = -bval->dela[Sisol] * vmole(V, mat0, mat1) * theta(V, mat0, mat1) *
			  vpow;
		right_side(row, sol, bval->rhs, tmp);
	}
	...
	if (model(V, mat0, mat1) & TIME)
	{
		if (injstr(V, mat0, mat1) != NULL)
			tpow = form_eval(injstr(V, mat0, mat1), total + bval->delta,
							 bval->cord);
		else
			tpow =
				A(V, mat0, mat1) * pow(t0(V, mat0, mat1) + total + bval->delta,
									   Tpow(V, mat0, mat1));
		tmp = bval->cpl * tpow;
		right_side(row, sol, bval->rhs, tmp);
	}
	...
	for (i = 0; i < nn; i++)
	{
		...
		if (methdata.defmodel == FULLCPL)
		{
			...
			for (j = 0; j < ndop; j++)
			{
				...
				a[vs][s][i] +=
					cpl * cs->nco[i] * tmp * cs->new[vs][i] * dact[s][s][i];
		...
		if (FALSE)
		{ /*(methdata.defmodel == FULLCPL) && (as != -1) ) {*/
			if (cs->type == TR)
			{
				/*trapazoidal rule setup*/
				...
				/*the time terms*/
				rhs[vs][i] -=
					cpl * (cs->nco[i] * (cs->new[as][i] - cs->newa[as][i]) -
			...
			else if (cs->type == BDF)
			{
				...
				/*right hand side terms*/
				rhs[vs][i] -=
					cs->nco[i] * cpl * (cs->new[as][i] - cs->newa[as][i]) -
...
void vacancy(char *par, struct par_str *param)
{
	...
	if (mat2 != -1)
	{
		...
		if (is_specified(param, "inj.str"))
		{
			...
			injstr(V, mat, mat2) = injstr(V, mat2, mat) =
...
void comp_vacparam(float temp)
{
	...
	for (mat1 = 0; mat1 < MAXMAT; mat1++)
	{
		...
		for (mat2 = 0; mat2 < MAXMAT; mat2++)
		{
			...
			Ksurf(V, mat1, mat2) =
				Ksurf0(V, mat1, mat2) * exp(-KsurfE(V, mat1, mat2) / Vt);
			Krat(V, mat1, mat2) =
				Krat0(V, mat1, mat2) * exp(-KratE(V, mat1, mat2) / Vt);
			Kpow(V, mat1, mat2) =
				Kpow0(V, mat1, mat2) * exp(-KpowE(V, mat1, mat2) / Vt);

			/*growth model for injection*/
			theta(V, mat1, mat2) =
				theta0(V, mat1, mat2) * exp(-thetaE(V, mat1, mat2) / Vt);
			Gpow(V, mat1, mat2) =
				Gpow0(V, mat1, mat2) * exp(-GpowE(V, mat1, mat2) / Vt);

			/*time dependent injection model*/
			A(V, mat1, mat2) = A0(V, mat1, mat2) * exp(-AE(V, mat1, mat2) / Vt);
			t0(V, mat1, mat2) =
				t00(V, mat1, mat2) * exp(-t0E(V, mat1, mat2) / Vt);
			Tpow(V, mat1, mat2) =
				Tpow0(V, mat1, mat2) * exp(-TpowE(V, mat1, mat2) / Vt);

			/*segregation terms*/
			Seg(V, mat1, mat2) =
				Seg0(V, mat1, mat2) * exp(-SegE(V, mat1, mat2) / Vt);
			Trn(V, mat1, mat2) =
				Trn0(V, mat1, mat2) * exp(-TrnE(V, mat1, mat2) / Vt);
		}
	}
}
