			   ...
			   double **dequ, double **rhs)
{
	...
	if ((vs != -1))
	{
		...
		for (i = 0; i < nn; i++)
		{
			...
			a[is][ps][i] -=
				cpl * (equil[is][i] * dequ[vs][i] + equil[vs][i] * dequ[is][i]);
...
void Iboundary(struct bound_str *bval)
{
	...
	if ((mat1 == Si) || (mat1 == GaAs) || ((mat0 != Si) && (mat1 == Poly)))
	{
		Sisol = 1;
		row = bval->loc[1][1];
	}
	...
	if (model(I, mat0, mat1) & RECOM)
	{
		...
		if (recstr(I, mat0, mat1) != NULL)
			K = form_eval(recstr(I, mat0, mat1), total + bval->delta,
						  bval->cord);
		else
	...
	if (model(I, mat0, mat1) & GROWTH)
	{
		...
		tmp = -bval->dela[Sisol] * vmole(I, mat0, mat1) * theta(I, mat0, mat1) *
			  vpow;
		right_side(row, sol, bval->rhs, tmp);
	}
	...
	if (model(I, mat0, mat1) & TIME)
	{
		if (injstr(I, mat0, mat1) != NULL)
			tpow = form_eval(injstr(I, mat0, mat1), total + bval->delta,
							 bval->cord);
		else
			tpow =
				A(I, mat0, mat1) * pow(t0(I, mat0, mat1) + total + bval->delta,
									   Tpow(I, mat0, mat1));
		tmp = bval->cpl * tpow;
		right_side(row, sol, bval->rhs, tmp);
	}
	...
	for (i = 0; i < nn; i++)
	{
		...
		if (methdata.defmodel == FULLCPL)
		{
			...
			for (j = 0; j < ndop; j++)
			{
				...
				a[is][s][i] +=
					cpl * cs->nco[i] * tmp * cs->new[is][i] * dequ[s][i];
		...
		if ((ts != -1) && ((mat == Poly) || (mat == Si) || (mat == GaAs)))
		{
			...
			dnda =
				(expa * (-cs->new_del * (ot[i] - tmp) + tmp / at) - tmp / at);
			dndb = (1.0 - expa) / at;
			...
			if (cs->type == TR)
			{
				...
				rhs[is][i] +=
					cpl * Ttot(mat) *
					(cs->nco[i] * cs->new[ts][i] - cs->oco[i] * cs->old[ts][i]);
			}
			else if (cs->type == BDF)
			{
				/*sum in the contribution of recombination*/
...
void foo2(ar1, ar2) double *ar1, *ar2;
{
	register int i;
	for (i = 0; i < nn; i++)
		z[i] = ar1[i] - ar2[i];
...
void interstitial(char *par, struct par_str *param)
{
	...
	if (mat2 != -1)
	{
		...
		if (is_specified(param, "rec.str"))
		{
			...
			recstr(I, mat, mat2) = recstr(I, mat2, mat) =
				salloc(char, strlen(tmp) + 1);
			strcpy(recstr(I, mat, mat2), tmp);
		}
		if (is_specified(param, "inj.str"))
		{
			tmp = get_string(param, "inj.str");
			injstr(I, mat, mat2) = injstr(I, mat2, mat) =
				salloc(char, strlen(tmp) + 1);
...
void comp_intparam(float temp)
{
	...
	for (mat1 = 0; mat1 < MAXMAT; mat1++)
	{
		...
		Dfrac[wi][mat1][CN] =
			Dfrac0[wi][mat1][CN] * exp(-DfracE[wi][mat1][CN] / Vt);
		Dfrac[wi][mat1][CM] =
			Dfrac0[wi][mat1][CM] * exp(-DfracE[wi][mat1][CM] / Vt);
		Dfrac[wi][mat1][CDM] =
			Dfrac0[wi][mat1][CDM] * exp(-DfracE[wi][mat1][CDM] / Vt);
		Dfrac[wi][mat1][CTM] =
			Dfrac0[wi][mat1][CTM] * exp(-DfracE[wi][mat1][CTM] / Vt);
		Dfrac[wi][mat1][CP] =
			Dfrac0[wi][mat1][CP] * exp(-DfracE[wi][mat1][CP] / Vt);
		Dfrac[wi][mat1][CDP] =
			Dfrac0[wi][mat1][CDP] * exp(-DfracE[wi][mat1][CDP] / Vt);
		Dfrac[wi][mat1][CTP] =
			Dfrac0[wi][mat1][CTP] * exp(-DfracE[wi][mat1][CTP] / Vt);
		...
		for (mat2 = 0; mat2 < MAXMAT; mat2++)
		{
			...
			Ksurf(I, mat1, mat2) =
				Ksurf0(I, mat1, mat2) * exp(-KsurfE(I, mat1, mat2) / Vt);
			Krat(I, mat1, mat2) =
				Krat0(I, mat1, mat2) * exp(-KratE(I, mat1, mat2) / Vt);
			Kpow(I, mat1, mat2) =
				Kpow0(I, mat1, mat2) * exp(-KpowE(I, mat1, mat2) / Vt);

			/*growth model for injection*/
			theta(I, mat1, mat2) =
				theta0(I, mat1, mat2) * exp(-thetaE(I, mat1, mat2) / Vt);
			Gpow(I, mat1, mat2) =
				Gpow0(I, mat1, mat2) * exp(-GpowE(I, mat1, mat2) / Vt);

			/*time dependent injection model*/
			A(I, mat1, mat2) = A0(I, mat1, mat2) * exp(-AE(I, mat1, mat2) / Vt);
			t0(I, mat1, mat2) =
				t00(I, mat1, mat2) * exp(-t0E(I, mat1, mat2) / Vt);
			Tpow(I, mat1, mat2) =
				Tpow0(I, mat1, mat2) * exp(-TpowE(I, mat1, mat2) / Vt);

			/*segregation terms*/
			Seg(I, mat1, mat2) =
				Seg0(I, mat1, mat2) * exp(-SegE(I, mat1, mat2) / Vt);
			Trn(I, mat1, mat2) =
				Trn0(I, mat1, mat2) * exp(-TrnE(I, mat1, mat2) / Vt);
		}
	}
}
