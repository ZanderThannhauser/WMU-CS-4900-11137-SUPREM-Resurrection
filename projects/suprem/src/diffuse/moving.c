...
void moving_bound(double dt, double scale)
{
	...
	for (i = 0; i < nn; i++)
		old[dsol][i] = mid[dsol][i] = new[dsol][i] = 0.0;
	...
	for (i = 0; i < np; i++)
	{
		for (j = 0; j < pt[i]->nn; j++)
		{
			for (k = j + 1; k < pt[i]->nn; k++)
			{
				...
				/*if the dot product of the velocities is negative - growing
				 * interface*/
						...
						case TWOD:
							...
							for (it = 0; it < num_tri_nd(nx1); it++)
							{
								...
								for (s = 0; s < 3; s++)
								{
									...
									oldln[0] +=
										pt[tail]->cord[1] - pt[head]->cord[1];
									oldln[1] +=
										pt[head]->cord[0] - pt[tail]->cord[0];

									/*at mid time...*/
									pt[head]->cord[0] +=
										scale * new[xsol][hdnd] * dt;
									pt[head]->cord[1] +=
										scale * new[ysol][hdnd] * dt;
									pt[tail]->cord[0] +=
										scale * new[xsol][tlnd] * dt;
									pt[tail]->cord[1] +=
										scale * new[ysol][tlnd] * dt;
									midln[0] +=
										pt[tail]->cord[1] - pt[head]->cord[1];
									midln[1] +=
										pt[head]->cord[0] - pt[tail]->cord[0];
									...
									/*at mid time...*/
									pt[head]->cord[0] +=
										sci * new[xsol][hdnd] * dt;
									pt[head]->cord[1] +=
										sci * new[ysol][hdnd] * dt;
									pt[tail]->cord[0] +=
										sci * new[xsol][tlnd] * dt;
									pt[tail]->cord[1] +=
										sci * new[ysol][tlnd] * dt;
									newln[0] +=
										pt[tail]->cord[1] - pt[head]->cord[1];
									newln[1] +=
										pt[head]->cord[0] - pt[tail]->cord[0];
