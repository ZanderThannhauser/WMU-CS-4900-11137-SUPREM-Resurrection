	...
	else
	{
		for (i = 0; i < nn; i++)
		{
			if (IS_SEMI(mat_nd(i)))
			{
				...
				dnet[psol][i] =
					-Ni(nd[i]->mater) * (noni[i] + 1 / noni[i]) / Vt;
			}
			else
			{
				net[i] = 0.0;
 ...
 *									*
 ************************************************************************/
void	 compute_sub(nv, temp, chem, active, dsub, net, dnet) int nv;
float	temp;
double **chem;
double **active;
double **dsub;
...
double **dnet;
{
	...
	for (i = 0; i < n_imp; i++)
	{
		...
		for (j = 0; j < nv; j++)
		{
			...
			ifr = Ichg[N][j] * Kcpl(I, mat, imp, N) +
				  Ichg[M][j] * Kcpl(I, mat, imp, M) +
				  Ichg[D][j] * Kcpl(I, mat, imp, D) +
				  Ichg[R][j] * Kcpl(I, mat, imp, R);
			pifr = pIchg[N][j] * Kcpl(I, mat, imp, N) +
				   pIchg[M][j] * Kcpl(I, mat, imp, M) +
				   pIchg[D][j] * Kcpl(I, mat, imp, D) +
				   pIchg[R][j] * Kcpl(I, mat, imp, R);

			vfr = Vchg[N][j] * Kcpl(V, mat, imp, N) +
				  Vchg[M][j] * Kcpl(V, mat, imp, M) +
				  Vchg[D][j] * Kcpl(V, mat, imp, D) +
				  Vchg[R][j] * Kcpl(V, mat, imp, R);
			pvfr = pVchg[N][j] * Kcpl(V, mat, imp, N) +
				   pVchg[M][j] * Kcpl(V, mat, imp, M) +
				   pVchg[D][j] * Kcpl(V, mat, imp, D) +
				   pVchg[R][j] * Kcpl(V, mat, imp, R);
			...
			dact[i][psol][j] = -active[i][j] *
							   (pifr * chem[isol][j] + pvfr * chem[vsol][j]) /
							   den2;
			...
			net[j] +=
				active[i][j] * (sg + ifr * chem[isol][j] + vfr * chem[vsol][j]);
			...
			dnet[psol][j] +=
				active[i][j] * (pifr * chem[isol][j] + pvfr * chem[vsol][j]);
		}
	}
}
#endif
