...
void set_X(GraphWidget gw)
{
	...
	wi->dev_info.axis_height = gw->graph.labelFont->max_bounds.ascent + gw->graph.labelFont->max_bounds.descent;

	wi->dev_info.title_width = XTextWidth(gw->graph.titleFont, "8", 1);
	wi->dev_info.title_height = gw->graph.labelFont->max_bounds.ascent + gw->graph.labelFont->max_bounds.descent;
	wi->max_segs = 100;
...
// int style;			/* Text style (above)     */
{
	...
	XDrawString(XtDisplay(gw), XtWindow(gw), loc_gc, rx, ry + bb.ascent, text, len);
