 ...
 * title in pixels.
 */
void DrawTitle(void *w, GraphWin * wi)
{
 ...
 */
void DrawGridAndAxis(void *w, GraphWin * wi)
{
	ENTER;
 ...
 */
void DrawData(void *w, GraphWin *wi)
{
    DataSet *temp;
 ...
 * title in pixels.
 */
void DrawLabel(void *gw, GraphWin * wi)
{
    LabelList *spot;
    int code1;
 ...
 * will fit are drawn.
 */
void DrawLegend(void *gw, GraphWin * wi)
{
	ENTER;
	DataSet *temp;
	int cnt;
	...
	for (temp = wi->Data; temp != NULL; temp = temp->next)
	{
		if ((temp->list) && (spot + wi->dev_info.axis_height + 2 < wi->XOppY))
		{
			if (temp->setName)
			{
				...
				wi->dev_info.xg_text(gw, wi->XOppX + wi->dev_info.bdr_pad, spot+wi->dev_info.bdr_pad, temp->setName, T_UPPERLEFT, T_AXIS);
			}
			else
			{
				if (8 > lineLen) lineLen = oneLen;
				DrawNumber(gw, wi, temp->setValue, wi->XOppX + wi->dev_info.bdr_pad, spot+wi->dev_info.bdr_pad, T_UPPERLEFT, 0);
			}
	...
	for(cnt = 0, temp = wi->Data; temp != NULL; temp = temp->next, cnt++)
	{
		...
		if ((temp->list) && (spot + wi->dev_info.axis_height + 2 < wi->XOppY))
		{
			...
			if (markFlag)
			{
				wi->dev_info.xg_dot(gw, leg_line.x1, leg_line.y1, P_MARK, cnt, cnt);
			}
			spot += 2 * wi->dev_info.axis_height;
		}
	}
...
int TransformCompute(GraphWin * wi)
{
	...
	*/
	...
	wi->XOrgX = wi->dev_info.bdr_pad + (8 * wi->dev_info.axis_width) + wi->dev_info.bdr_pad;
	...
	*/
	...
	wi->XOppY = wi->dev_info.area_h - wi->dev_info.bdr_pad - wi->dev_info.axis_height - wi->dev_info.bdr_pad;
...
void do_redraw(GraphWidget userdata, GraphWin * wi)
{
