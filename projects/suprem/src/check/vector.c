...
char *get_solval(float *val, int type)
{
	...
	switch (type)
	{
		...
		case iG:
			index = imptosol[type];
			if (index == -1)
				return ("The value is not available for plotting\n");
			for (i = 0; i < nn; i++)
				val[i] = sol_nd(i, index);
			return (NULL);
			break;
		...
		case H:
			...
			if (index == -1)
			{
				...
				index = imptosol[Psi];
				for (i = 0; i < nn; i++)
					val[i] = exp(sol_nd(i, index) / Vt) * Ni(mat_nd(i));
			}
			else
				for (i = 0; i < nn; i++)
					val[i] = sol_nd(i, index);
		...
		case QFN:
			...
			isol = imptosol[N];
			for (i = 0; i < nn; i++)
				val[i] = Phi_n(sol_nd(i, isol), sol_nd(i, psol), Ec[i],
							   mat_nd(i), &d1, &d2);
			break;
		...
		case QFP:
			...
			isol = imptosol[H];
			for (i = 0; i < nn; i++)
				val[i] = Phi_p(sol_nd(i, isol), sol_nd(i, psol), Ev[i],
							   mat_nd(i), &d1, &d2);
			break;
		...
		case ECON:
			index = imptosol[Psi];
			for (i = 0; i < nn; i++)
				val[i] = -sol_nd(i, index) + Ec[i];
			break;

		case EVAL:
			index = imptosol[Psi];
			for (i = 0; i < nn; i++)
				val[i] = -sol_nd(i, index) + Ev[i];
			break;
#endif
...
char *vfunc(float *val, int type, struct vec_str *expr)
{
	...
	switch (type)
	{
		...
		case DO_ACTIVE:
			...
			/*relate a stored concentration to the active substitutional
			 * portion*/
			switch (imp)
			{
				case As:
		...
		case DATA:
			...
			if (lex == NULL)
			{
				if (rex->type != STRING)
				{
					fprintf(stderr, "error in data function, argument must be "
									"a filename\n");
				}
				else
				{
			...
			else
			{
				...
				{
					if (read_data(val, lex->value.sval, (int)rex->value.dval,
								  0) == -1)
						fprintf(stderr, "error in data function, empty file");
				}
			}
...
int sol_values(char *s, struct tok_str *tok)
{
	...
	else if (substring("x.veloc", s) || (!strcmp("v.x", s)) ||
			 (!strcmp("xv", s)) || (!strcmp("vx", s)))
	{
		tok->type = SOLVAL;
		tok->value.ival = XVEL;
		return (0);
	}
	else if (substring("y.veloc", s) || (!strcmp("v.y", s)) ||
			 (!strcmp("yv", s)) || (!strcmp("vy", s)))
	{
		tok->type = SOLVAL;
		tok->value.ival = YVEL;
		return (0);
...
void RoughGrad(int which, float *vector)
{
	...
	for (in = 0; in < nn; in++)
	{
		...
		for (i = 0; i < nnb; i++)
		{
			...
			accum[in] +=
				ehed * ((vector[in] - vector[nb]) / len) *
				(cordinate(pt_nd(in), which) - cordinate(pt_nd(nb), which)) /
				len;
			wt += ehed;
		}
		if (wt != 0.0)
