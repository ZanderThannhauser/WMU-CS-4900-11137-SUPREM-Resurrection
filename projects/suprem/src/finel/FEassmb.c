 ...
 *									*
 ************************************************************************/
void FEassmb(int *ia, int aoff, double *a, double *rhs)
{
	...
	for (ie = 0; ie < FEne; ie++)
	{
		...
		if (!dummy)
		{
			...
			(*(desc->bound))(stiff, wrhs, xl, disp, desc->coeff, pie->mat,
							 fixity, bc, ie, NotYet);
			NotYet = 0;
		}
			...
			int eqns[MELDOF], int fixity[MELDOF], int dummy)
{
	...
	for (i = 0; i < eldof; i++)
	{
		...
		for (j = 0; j < i; j++)
		{
			...
			if (eqns[j] < eqns[i])
			{ /*global order same as local order*/
				aloc = iaja(ia, aoff, eqns[i], eqns[j]);
				row = i;
				col = j;
			}
			else
			{ /*global order reversed*/
...
void dump_stiff(stiff, rhs, nel, stride, offset) double *stiff, *rhs;
