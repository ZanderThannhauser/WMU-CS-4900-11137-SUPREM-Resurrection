	...
	for (bp1 = bnd->next; (bp1->next != bnd); bp1 = bp1->next)
	{
		if (dabs(bp1->ang - PI) < 1.0e-3)
		{
			...
			if (lil(x, dx, q, dq, alp))
				panic("rect_cnt: parallel lines?!?");
			if ((alp[0] < 0.0) || (alp[0] > 1.0))
				panic("new node not on old edge");
			q[0] = alp[0] * dx[0] + x[0];
			q[1] = alp[0] * dx[1] + x[1];
...
void rect_tri(int r, int rbn)
{
	...
	while (sreg[r]->len > 4)
	{
		...
		bpi2 = NULL;
		bn = NULL;
		for (f = 1, bp = sreg[r]->bnd; f || (bp != sreg[r]->bnd);
			 f = 0, bp = bp->next)
		{
	...
	if (sreg[r]->len != 4)
		panic("supposed to quit with quads!!");
	bp = quadsplit(sreg[r]);
