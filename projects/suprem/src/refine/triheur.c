...
struct LLedge *chop(struct sreg_str *r, int must)
{
	...
	for (f = 1, bp = mina; (bp != mina) || f; bp = bp->gt, f = 0)
	{
		...
		p[0] = xcord(pt_nd(i));
		p[1] = ycord(pt_nd(i));
		dp[0] = xcord(pt_nd(k)) - p[0];
		dp[1] = ycord(pt_nd(k)) - p[1];
		q[0] = xcord(pt_nd(j));
		q[1] = ycord(pt_nd(j));
		...
		for (triok = TRUE, bq = bp->next; triok && (nF(bq) != i); bq = bq->next)
		{
			...
			dq[0] = xcord(pt_nd(l)) - q[0];
			dq[1] = ycord(pt_nd(l)) - q[1];

			/*if parallel, no problem...*/
			if (lil(p, dp, q, dq, alp))
				continue;

			/*if crossing isn't within the arc at middle*/
			if ((alp[0] < -EPS) || (alp[0] > 1.0 + EPS))
				continue;
		...
		if (triok)
		{
			if ((g = good_tri(i, j, k)) > mgeo)
			{
				...
				/*...Is it too big? */
				lej = ndist(i, k);
				h1 = dmin(get_space(pt_nd(i)), lej) /
					 dmax(get_space(pt_nd(i)), lej);
				h2 = dmin(get_space(pt_nd(k)), lej) /
					 dmax(get_space(pt_nd(k)), lej);
...
int divide(struct sreg_str *r, struct LLedge **lep1, struct LLedge **lep2)
{
		 ...
		 bp1 = bp1->lt, f1 = 0)
	{
			 ...
			 bp2 = bp2->lt, f2 = 0)
		{
			...
			if (bp2 == bp1 || bp1 == bp2->next || bp2 == bp1->next)
				continue;
			...
			if (ang1 < piby4 || ang2 < piby4)
				continue;
			if (bp1->ang - ang1 < piby4 || bp2->ang - ang2 < piby4)
				continue;

			/*...Check for [near] crossings */
			if (sk_cross(r, bp1, bp2, FALSE) < 0.5)
				continue;
				...
				int rect)
{
	...
	float		   alph[MAXDIM],
		pval[MAXDIM]; /* Present & prev projection of node on split */
	...
	dvpram(get_space(pt_nd(nB(lp1))), get_space(pt_nd(nB(lp2))), lej, &nnew,
		   &ratio, &first);
	near = MAXFLOAT;
	...
	for (sign = 1, bp = lp1->next; bp != end; bp = bp->next)
	{
		...
		if (bp == lp2)
		{ /* Change sign crossing the equator. */
			pval[0] = 0.5;
			pval[1] = sign = -1;
			continue;
		}
		...
		if (pval[1] * alph[1] < 0)
		{ /* Then check it intersects inside */
			inter =
				(pval[1] * alph[0] - alph[1] * pval[0]) / (pval[1] - alph[1]);
		...
		if (sign * alph[1] < 0)
			continue; /* Skip wraparound points. */
		if (alph[0] < -EPS || alph[0] > 1 + EPS)
			continue; /* Not between ends. */
		if (rect && (alph[0] < EPS || alph[0] > 1 - EPS))
			continue;
