...
int sp_reg(struct LLedge *lep1, struct LLedge *lep2, struct LLedge *bp_look)
{
	...
	if (bp_look == NULL)
	{
		for (llen = 0, bp = lep2; bp != lep1; bp = bp->next)
			llen++;
		for (slen = 0, bp = lep1; bp != lep2; bp = bp->next)
			slen++;
		if (llen < slen)
		{
			bp = lep1;
	...
	else
	{
		...
		for (bp = lep2; bp != lep1; bp = bp->next)
			fnd = fnd || bp == bp_look;
