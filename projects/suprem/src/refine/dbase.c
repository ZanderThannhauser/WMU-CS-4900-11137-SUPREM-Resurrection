 ...
 *----------------------------------------------------------------------*/
int cr_sreg(int mat) /* Material number of region. */
{
	int ir = nsreg;
...
int cr_tri(int rn, int ir)
{
	...
	if (is_surf(pt_nd(nv[1])) && is_surf(pt_nd(nv[2])))
		nb[0] = EXPOSED;
	if (is_surf(pt_nd(nv[0])) && is_surf(pt_nd(nv[2])))
		nb[1] = EXPOSED;
...
void ad_edge(int ir, int ie, struct LLedge *lep, int iscc, int pos)
{
	...
	if (!sreg[ir] || !edg[ie])
		panic("Bad args to ad_edge");
	if (sreg[ir]->bnd && !lep)
		panic("No position given to ad_edge");
...
for (i = 0; i < nvrt; i++)
{
	if ((nd[tri[t]->nd[i]]->pt != p) && (nd[tri[t]->nd[i]]->pt != p2))
		lj = i;
}
