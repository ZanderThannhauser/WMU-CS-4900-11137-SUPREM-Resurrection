...
void make_surf(struct surf_info **v, double ang, double lat)
{
	...
	/*2020: initialize lft.top[0] to zero? Is that what they wanted 40 years
	 * ago?*/
	/*before angular rotation, get the materials at left and right*/
	lft.top[0] = 0, lft.rht[0] = lft.lft[0] = pt[s[0].p]->cord[0] + ERR;
	...
	HERE;
	rht.top[0] = 0, rht.lft[0] = rht.rht[0] = pt[s[ns - 1].p]->cord[0] - ERR;
	get_mat(&rht, TRUE);
