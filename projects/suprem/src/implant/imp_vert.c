...
/* 2020 includes:*/
#include "debug.h"
#include "../implant/imp_prson.h"
#include "../implant/imp_lat.h"
#include "imp_vert.h"
/* end of includes*/
...
double func(double l[MAXDIM], double pt[MAXDIM], struct imp_info *dat)
{
	double ret = (imp_vert((l[0] - pt[0]) * 1.0e4, dat) *
				  imp_latr((l[1] - pt[1]) * 1.0e4, dat));
	return ret;
}

double simp_int(double l[MAXDIM], double a[MAXDIM], double b[MAXDIM],
				double eps, struct imp_info *dat)
{
	int	i;
	double s, sbar;
