...
#include "suprem/include/FEmath.h" /* for FErel */
 ...
 *									*
 ************************************************************************/
void method(char *par, struct par_str *param)
{
	char *s;
	float tmpf;
	int   imp = -1;
	...
	if (TURNEDON(param, "vacancies"))
		imp = V;
	if (TURNEDON(param, "interstitial"))
		imp = I;
	if (TURNEDON(param, "arsenic"))
		imp = As;
	if (TURNEDON(param, "phosphorus"))
		imp = P;
	if (TURNEDON(param, "antimony"))
		imp = Sb;
	if (TURNEDON(param, "boron"))
		imp = B;
	if (TURNEDON(param, "oxidant"))
		imp = O2;
	if (TURNEDON(param, "velocity"))
		imp = XVEL;
	if (TURNEDON(param, "traps"))
		imp = T;
	if (TURNEDON(param, "gold"))
		imp = Au;
	if (TURNEDON(param, "psi"))
		imp = Psi;
	if (TURNEDON(param, "cesium"))
		imp = Cs;
	if (TURNEDON(param, "electron"))
		imp = N;
	if (TURNEDON(param, "hole"))
		imp = H;
	if (TURNEDON(param, "circuit"))
		imp = CKT;
	if (TURNEDON(param, "gallium"))
		imp = Ga;
	if (TURNEDON(param, "beryllium"))
		imp = iBe;
	if (TURNEDON(param, "magnesium"))
		imp = iMg;
	if (TURNEDON(param, "selenium"))
		imp = iSe;
	if (TURNEDON(param, "isilicon"))
		imp = iSi;
	if (TURNEDON(param, "tin"))
		imp = iSn;
	if (TURNEDON(param, "germanium"))
		imp = iGe;
	if (TURNEDON(param, "zinc"))
		imp = iZn;
	if (TURNEDON(param, "carbon"))
		imp = iC;
	if (TURNEDON(param, "generic"))
		imp = iG;
	...
	if (imp != -1)
	{
		...
		/*the error bounds*/
		if (is_specified(param, "rel.error"))
			LTE[imp] = get_float(param, "rel.error");
		if (is_specified(param, "abs.error"))
			ABE[imp] = get_float(param, "abs.error");
	}
	...
	if (TURNEDON(param, "guass"))
		methdata.block = SOR;
	if (TURNEDON(param, "cg"))
		methdata.block = ICCG;

	/*how often do we factor the matrix in time solutions???*/
	if (TURNEDON(param, "time"))
		methdata.factor = RF_TIM;
	if (TURNEDON(param, "err"))
		methdata.factor = RF_ERR;
	if (TURNEDON(param, "newton"))
		methdata.factor = RF_ALL;

	/*what should we use as the preconditioner?*/
	if (TURNEDON(param, "diag"))
		methdata.fill = DIAG;
	if (TURNEDON(param, "knot"))
		methdata.fill = NONE;
	if (TURNEDON(param, "full.fac"))
		methdata.fill = FULL;

	/*how many back vectors for cg methods*/
	if (is_specified(param, "back"))
		methdata.back = get_int(param, "back");
	...
	if (TURNEDON(param, "trbdf"))
		methdata.timemeth = TRBDF;
	...
	if (mode != ONED)
	{
		if (TURNEDON(param, "erfc"))
			oxide_model = O_ANALYTIC;
		if (TURNEDON(param, "erf1"))
			oxide_model = O_ERF1;
		if (TURNEDON(param, "erf2"))
			oxide_model = O_ERF2;
		if (TURNEDON(param, "erfg"))
			oxide_model = O_ERFG;
		if (TURNEDON(param, "vertical"))
			oxide_model = O_VERTICAL;
		if (TURNEDON(param, "compress"))
			oxide_model = O_ELASTIC;
		if (TURNEDON(param, "viscous"))
	...
	if (is_specified(param, "grid.oxide"))
	{
		...
		{
			fprintf(stderr,
					"grid.oxide (%g) below 10A is unsafe, resetting to 0.1u\n",
					oxide_grid);
			oxide_grid = 0.1e-4;
		}
	}
	...
	{
		fprintf(stderr,
				"Bad values for oxide.early (%g) and oxide.late (%g), "
				"resetetting\n",
...
void clean_list(short *taken, int *num)
{
	int i, j, swap, ndup;
