...
#include "suprem/include/matrix.h" /* For the old, new areas */
...
#define dot(A, B)                                                              \
	((mode == ONED) ? ((A)[X] * (B)[X]) : ((A)[X] * (B)[X] + (A)[Y] * (B)[Y]))
#define abs(X) (((X) >= 0) ? (X) : -(X))

#define vertex(t, i) tri[t]->nd[i]
#define nbrtri(t, i) tri[t]->nb[i]
#define point(n) nd[n]->pt
#define sameRegion(a, b)                                                       \
	((a == b) || (a >= 0) && (b >= 0) && (reg_tri(a) == reg_tri(b)))
 ...
 *----------------------------------------------------------------------*/
int grid_add(double dt) /* Returns fix_conn */
{
	int fix_conn = 0;
	...
	switch (mode)
	{
		case TWOD:
			for (ie = 0; ie < ne; ie++)
			{
				...
				p[X] = xcord(p1) - xcord(p0);
				p[Y] = ycord(p1) - ycord(p0);
				q[X] = xcord(p2) - xcord(p0);
				q[Y] = ycord(p2) - ycord(p0);
				...
				else
				{
					...
					if (discrim > 0)
					{
						...
						if (root1 > 0 && root1 < minRoot)
							minRoot = root1;
						root2 = (-pB - discrim) / (2 * pA);
						if (root2 > 0 && root2 < minRoot)
							minRoot = root2;
					}
				}
 ...
 *----------------------------------------------------------------------*/
void ChooseKillNodes(double dt) /* time step */ { return; }
...
#define pvel(A, B) pt[A]->vel[B]
	...
	for (ie = 0; ie < ned; ie++)
	{
		...
		lrel[X] = xcord(pl[0]) - dt * pvel(pl[0], X) - xcord(pl[1]) +
				  dt * pvel(pl[1], X);
		lrel[Y] = ycord(pl[0]) - dt * pvel(pl[0], Y) - ycord(pl[1]) +
				  dt * pvel(pl[1], Y);
		lrel[Z] = zcord(pl[0]) - dt * pvel(pl[0], Z) - zcord(pl[1]) +
				  dt * pvel(pl[1], Z);
		...
		for (ie = 0; ie < ne; ie++)
		{
			...
			for (j = 0; j < 3; j++)
			{
				...
				if (pt[ip]->vel[X] == 0 && pt[ip]->vel[Y] == 0 &&
					...
					pt[ip2]->vel[X] == 0 && pt[ip2]->vel[Y] == 0)
					continue;

				/* Check if the triangle looks bad after update */
				if ((tt = BadObtuse(xcord(ip), ycord(ip), xcord(ip1),
									ycord(ip1), xcord(ip2), ycord(ip2))) > 0.5)
					continue;
				...
				if (BadObtuse((float)(xcord(ip) - dt * pt[ip]->vel[X]),
							  ...
							  (float)(ycord(ip2) - dt * pt[ip2]->vel[Y])) < tt)
					continue;
