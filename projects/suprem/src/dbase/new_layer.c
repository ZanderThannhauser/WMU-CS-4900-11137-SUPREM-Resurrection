...
#define dot(A, B)                                                              \
	((mode == ONED) ? ((A)[0] * (B)[0]) : ((A)[0] * (B)[0] + (A)[1] * (B)[1]))
...
void NativeOxide(double thick) /* That's me */
{
	...
	switch (mode)
	{
		...
		case TWOD:
			for (ir = 0; ir < nreg; ir++)
			{
				if ((mat_reg(ir) != Si) && (mat_reg(ir) != Poly))
					continue;
				sr = reg[ir]->sreg;

				/*loop through all the edges in this region looking for
				 * exposed*/
				bnd = sreg[sr]->bnd;
				ende = NULL;
				for (g = 1, beg = bnd; g || beg != bnd; g = 0, beg = beg->next)
				{
					if (!ask(edg[beg->edge], ESURF))
						continue;

					/*find the end*/
					for (ende = beg; ask(edg[ende->edge], ESURF);
						 ende = ende->next)
						;
...
int new_layer(int Msil, int Mox, double oxide_grid, double dt)
{
	...
	for (io = 0; io < nn; io++)
	{
		...
		if ((fig * len_edg(eperp) >= oxide_grid) && (dt * magv > 20.0e-8))
		{
			/*split this edge*/
 ...
 *----------------------------------------------------------------------*/
int exposed(int in) /* node number */
{
	int ix, ixx, jin, j, nbr;

	if (node_mat(in, GAS) < 0)
		return (0);
...
int   split_thick(n, Mox, Msil, vdisp) int n;
int   Mox, Msil;
float vdisp[MAXDIM];
{
	...
	dox = rough_perp(nd[n]->pt, Mox, SilToOx);
	if (dox < 0)
		return (0);

	/* Provide a little hysteresis by adding grid anywhere between 0.25 and 1*/
	/* Danger: don't want slightly less thick oxides triggering just after field
	 * oxide */
...
float rough_perp(int p, int matl, float dir[MAXDIM])
{
	...
	else
	{
		fprintf(
			stderr,
			"Warning: unknown spacing for point %d[%d] in direction (%e, %e)\n",
			p, matl, dir[0], dir[1]);
		return (-1);
	}
}
		...
		case TWOD:
			...
			for (it = 0; it < num_tri_nd(n); it++)
			{
				...
				for (j = v; (j = (j + 1) % 3) != v;)
					if (tri[t]->nb[j] == NEUTRAL)
					{
						...
						/*If there is more than one, well they should be
						 * parallel */
						is_refl++;
						p1 = nd[n]->pt;
			...
			for (it = 0; it < num_tri_nd(n); it++)
			{
				...
				for (j = v; (j = (j + 1) % 3) != v;)
				{
					...
					if (matl > GAS)
					{
						if (nbr < 0 || mat_reg(reg_tri(nbr)) != matl)
							continue;
					}
					else if (matl == GAS || matl == -1)
					{
						/*Not just any negative neighbor, would get 45o at
						 * corners*/
						if (nbr != EXPOSED)
							continue;
					}
					...
					else
					{
						fprintf(
							stderr,
							"local_normal: more than two edges at a node?\n");
					}
				}
			}

			/*Now subtract out the component perpendicular to the reflection
			 * axis*/
			if (is_refl)
			{
				cosf = 1;
