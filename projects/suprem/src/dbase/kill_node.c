 ...
 * time we do this.
 *----------------------------------------------------------------------*/
void kill_node(int in) /* Number of node to operate on */
{
	register int i, j;
	...
	{
		if (maxn != 2)
			panic("can't do node removal in one dimension");
	}
	else if (mode == TWOD)
	{
		if (maxn < 3)
		{
			while (num_tri_nd(in) > 0)
				rem_ele(tri_nd(in, 0), FALSE);
			return;
		}
 ...
 * for the next node.
 *----------------------------------------------------------------------*/
void trotate(int ie, int iv, int clock, int *Pje, int *Poje, int *Pj, int *Pjj)
{
	...
	for (oje = ie, j = iv, jj = (j + delta) % 3;;)
	{
		...
		if ((je < 0) || (je == ie) || (tri[je]->regnum != ir))
			break;
