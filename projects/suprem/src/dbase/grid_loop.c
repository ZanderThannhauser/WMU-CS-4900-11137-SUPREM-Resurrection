...
int DetectLoop()
{
	...
	for (pass = 0, didSomething = 1; didSomething; pass++)
	{
		didSomething = 0;
		if (pass > 10)
			panic("DetectLoop infinite looped!");
		...
		while (done_tri(ie))
		{
			...
			for (j = 0; j < num_face(ie); j++)
			{
				...
				nbj = neigh_fc(ie, j);
				if (nbj >= 0 && reg_tri(nbj) == reg_tri(ie))
					continue;
				...
				if (dsin > 0)
				{
					...
					if (nbj < 0)
					{
						...
						if (tje >= 0)
						{
							fprintf(stderr, "Intrusion with intervening layer "
											"not yet done\n");
							continue;
						}
						if (tje != nbj)
							fprintf(
								stderr,
								"Warning: intrusion with inhomogeneous BC\n");
						didSomething = 1;
						fix_conn++;
						if (verbose >= V_BARF)
					...
					else
					{
						...
						if (tje != nbj)
						{
							fprintf(stderr, "Multiple triangle intrusion case "
											"to be completed\n");
							continue;
						}
						didSomething = 1;
				...
				else
				{
					...
					if (dcos < gloop_emax)
					{
						if (ie != toje)
						{
							fprintf(
								stderr,
								"Can't do multiple triangle extrusion yet\n");
							continue;
						}
					}
					...
					else
					{
						...
						/* Look back */
						trotate(ie, (j + 2) % 3, 1, &tttje, &tttoje, &tttj,
								&tttjj);
						...
						/* Try to distinguish between a 90 corner in the
						 * boundary */
						/* and a 90 jag in the boundary */
						cando = 0;
						if (dot(df, dnf) > 0 && cross(dnf, df) > 0 &&
							dot(db, dnb) > 0)
							cando = 1;
						if (dot(db, dnb) > 0 && cross(db, dnb) > 0 &&
							dot(df, dnf) > 0)
							cando = 1;
						if (!cando)
							continue;
					...
					if (nbj >= 0 && tje >= 0)
					{
						...
						{
							fprintf(stderr,
									"Can't decide whether to give %s triangle "
									"to %s or %s\n",
									MatNames[iem] + 1,
									MatNames[mat_tri(nbj)] + 1,
									MatNames[mat_tri(tje)] + 1);
							continue;
						}
						...
						/* Rename its nodes */
						vert_tri(ie, 0) =
							node_mat(vert_tri(ie, 0), mat_tri(ie));
						vert_tri(ie, 1) =
							node_mat(vert_tri(ie, 1), mat_tri(ie));
						vert_tri(ie, 2) =
							node_mat(vert_tri(ie, 2), mat_tri(ie));
