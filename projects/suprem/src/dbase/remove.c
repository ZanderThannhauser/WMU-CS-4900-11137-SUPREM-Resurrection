		...
		if (n >= 0)
		{
			...
			if (expos)
			{
				...
				for (k = 0; k < num_nd_fc(n, j); k++)
				{
					...
					if (node_mat(n1, GAS) == -1)
						(void)mk_nd(pt_nd(n1), GAS);
				}
			}
...
#define mag(A)                                                                 \
	((mode == ONED) ? (sqrt(A[0] * A[0])) : sqrt(A[0] * A[0] + A[1] * A[1]))
...
void rem_edg(int ie)
{
	...
	if (l + num_nd(p2) > 2)
	{
		if (dead_nd(n1) || dead_nd(n2))
			return;
		for (i = 0; i < num_ele_edg(ie); i++)
			if (dead_tri(ele_edg(ie, i)))
				return;
...
void rem_edg_end(int ie, int nr)
{
	...
	if (l > 2)
		panic("too many triangles depend on edge");
	for (i = 0; i < l; i++)
		rem_ele(dl[i], FALSE);
	if (!dead_nd(n1))
		rem_1nd(n1);
