...
char *Mnames[MAXMAT] = {"AIR",  "OXID", "NTRD", "SI",
						"POLY", "OXIN", "METL", "RST"};
...
void write_SIMPLPolygon(FILE *SIMPLfile)
{
	...
	if ((poly_data = (struct d_str *)malloc(np * sizeof(struct d_str))) == NULL)
	{
	...
	for (ir = 0; ir < nreg; ir++)
	{
		 ...
		 */
		if ((poly_array = (struct tmp_str *)malloc(
				 index * sizeof(struct tmp_str))) == NULL)
		{
		...
		{
			topleft =
				(poly_array[(i + 1) % index].x - poly_array[i].x > 0.001) &&
				(poly_array[(i - 1 + index) % index].y - poly_array[i].y >
				 0.001);
			topright =
				(poly_array[i].x - poly_array[(i - 1 + index) % index].x >
				 0.001) &&
				(poly_array[(i + 1) % index].y - poly_array[i].y > 0.001);
			bottomleft =
				(poly_array[(i - 1 + index) % index].x - poly_array[i].x >
				 0.001) &&
				(poly_array[i].y - poly_array[(i + 1) % index].y > 0.001);
			bottomright =
				(poly_array[i].x - poly_array[(i + 1) % index].x > 0.001) &&
				(poly_array[i].y - poly_array[(i - 1 + index) % index].y >
				 0.001);

			/* the magic check for points to delete */
			if (!((((bottom - poly_array[i].y) < 0.001) && (!bottomleft) &&
				   (!bottomright)) ||
				  (((poly_array[i].x - left) < 0.001) &&
				   ((bottom - poly_array[i].y) > 0.001) && (!topleft) &&
				   (!bottomleft)) ||
				  (((right - poly_array[i].x) < 0.001) &&
				   ((bottom - poly_array[i].y > 0.001)) && (!topright) &&
				   (!bottomright))))
			{
				count++;
					 ...
					 int ylocsize)
{
	...
	(void)fill_grid(xlocsize, ylocsize, xloc, yloc, data, z, Si, 0,
					xlocsize - 1, 0, ylocsize - 1, min_value);
