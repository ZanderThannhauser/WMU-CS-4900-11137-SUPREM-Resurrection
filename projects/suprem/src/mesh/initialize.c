...
#include "suprem/include/material.h" /* For B, P, Sb, As */
...
void initialize(char *par, struct par_str *param)
{
	...
	if (is_specified(param, "phosphorus") && get_bool(param, "phosphorus"))
		imp = P;
	if (is_specified(param, "gold") && get_bool(param, "gold"))
		imp = Au;
	if (is_specified(param, "gallium") && get_bool(param, "gallium"))
		imp = Ga;
	...
	if (is_specified(param, "generic") && get_bool(param, "generic"))
		imp = iG;
	ivratio =
		is_specified(param, "interval.r") ? get_float(param, "interval.r") : 0;
