...
#define EC(x)                                                                  \
	if ((erret = (x)))                                                         \
	{                                                                          \
		fprintf(stderr, "Pisces mesh i/o error: %s\n", erret);                 \
		return (-1);                                                           \
	}
...
int pi_write(char *name, int show)
{
	...
	{
		fprintf(stderr,
				"pi_write:can only export two dimensional structures\n");
	}
	...
	for (i = 0; i < nreg; i++)
		if (RECOG(mat_reg(i)))
			fprintf(lu, "%d ", corrmat[mat_reg(i)]);
	fprintf(lu, "\n");
...
char *pi_elect(int *nb, int *nelect, int **nbc, int **ietype)
{
	...
	Pnbc = scalloc(int, 2 * np);
	*nbc = Pnbc;
	Pietype = scalloc(int, 2 * np);
	*ietype = Pietype;
...
int ShowElect(int show, int Pnelect, int Pnb, int *Pnbc, int *Pietype)
{
	...
	int			i, ie;
	float *		ac, txmin, txmax, tymin, tymax, xslop, yslop;
...
char *pi_dop(float **r1, float **tconc)
{
	...
	*tconc = Ptconc;
	...
	if (imptosol[As] != -1)
		strcat(SelectBuf, "active(arsenic) + ");
	if (imptosol[B] != -1)
		strcat(SelectBuf, "active(boron) + ");
	if (imptosol[Sb] != -1)
		strcat(SelectBuf, "active(antimony) + ");
	if (imptosol[P] != -1)
		strcat(SelectBuf, "active(phosphorus) + ");
	strcat(SelectBuf, "0 )");
	do_str(SelectBuf);
