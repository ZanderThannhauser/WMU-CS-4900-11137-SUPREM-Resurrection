...
#include "suprem/include/impurity.h" /* For B, P, Sb, As */
 ...
 *									*
 ************************************************************************/
void profile(char *par, struct par_str *param)
{
	register int i;
	char *		 infile;
	float *		 val;
	...
	if (is_specified(param, "boron") && get_bool(param, "boron"))
		imp = B;
	if (is_specified(param, "antimony") && get_bool(param, "antimony"))
		imp = Sb;
	if (is_specified(param, "arsenic") && get_bool(param, "arsenic"))
		imp = As;
	if (is_specified(param, "phosphorus") && get_bool(param, "phosphorus"))
		imp = P;
	if (is_specified(param, "beryllium") && get_bool(param, "beryllium"))
		imp = iBe;
	if (is_specified(param, "magnesium") && get_bool(param, "magnesium"))
		imp = iMg;
	if (is_specified(param, "selenium") && get_bool(param, "selenium"))
		imp = iSe;
	if (is_specified(param, "isilicon") && get_bool(param, "isilicon"))
		imp = iSi;
	if (is_specified(param, "tin") && get_bool(param, "tin"))
		imp = iSn;
	if (is_specified(param, "germanium") && get_bool(param, "germanium"))
		imp = iGe;
	if (is_specified(param, "zinc") && get_bool(param, "zinc"))
		imp = iZn;
	if (is_specified(param, "carbon") && get_bool(param, "carbon"))
		imp = iC;
	if (is_specified(param, "generic") && get_bool(param, "generic"))
		imp = iG;
	if (is_specified(param, "interstitial") && get_bool(param, "interstitial"))
		imp = I;
	if (is_specified(param, "vacancy") && get_bool(param, "vacancy"))
		imp = V;
	...
	for (i = 0; i < nn; i++)
	{
		...
		if ((x >= data[2 * 0 + 0]) && (x <= data[2 * (cnt - 1) + 0]))
		{
			...
			if (j < cnt - 1)
			{
				...
				y = (x - data[2 * j + 0]) /
					(data[2 * (j + 1) + 0] - data[2 * j + 0]);
				y = (data[2 * (j + 1) + 1] - data[2 * j + 1]) * y +
					data[2 * j + 1];
