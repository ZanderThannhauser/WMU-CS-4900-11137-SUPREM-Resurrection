...
#include "suprem/include/global.h" /* For min() */
 ...
 *----------------------------------------------------------------------*/
void rline(char *par, struct par_str *param)
{
	int	i, j, dim, nu;
	double lloc, spa;
	char * s;
	...
	if (get_bool(param, "x.direction"))
		dim = 0;
	if (get_bool(param, "y.direction"))
		dim = 1;
	if (get_bool(param, "z.direction"))
		dim = 2;
...
void rregion(char *par, struct par_str *param)
{
	...
	/* number into ureg[.][TYPLOC] below. That's the way this started, but
	 * users*/
...
void redge(char *par, struct par_str *param)
{
	...
	if (chosen("backside"))
		uedge[nuj - 1][TYPLOC] = BC_OFFSET + 1;
	if (chosen("reflecting"))
		uedge[nuj - 1][TYPLOC] = BC_OFFSET;
